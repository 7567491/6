...
class RecommendRelation extends ModelBasic
{
    ...
    public static function getRelationList($recommend_id, $type, $imagetype, $limit,$is_member)
    {
        ...
        if ($type == 0 || $type == 8){
            ...
            $list = $model->limit($limit)->field(['p.id','p.pink_money','p.is_light','p.light_type','p.is_mer_visible', 'p.is_pink','p.sort','p.title', 'p.image', 'p.abstract', 'p.label', 'p.image', 'p.money', 'p.pay_type', 'p.type as special_type', 'a.link_id','a.add_time','p.browse_count','p.fake_sales','p.member_pay_type', 'p.member_money'])
                    ->select();
        }elseif ($type == 4){
            $list = self::where('a.recommend_id', $recommend_id)
                ...
                ->where(['p.is_show' => 1, 'p.is_del' => 0])
                ->field(['p.id','p.price', 'p.store_name', 'p.image','p.sales','p.ficti', 'p.store_info', 'p.keyword', 'p.vip_price', 'p.member_pay_type', 'a.link_id','a.add_time'])
                ->select();
        }elseif($type == 5){
            //热门直播
            ...
            $list = self::where('a.recommend_id', $recommend_id)
                ...
                ->where(['t.is_show' => 1, 't.is_del' => 0,'t.type'=>$types])
                ->field(['t.id','t.title','t.type', 't.image', 't.item_number','t.answer','t.money', 't.pay_type', 't.is_show', 'a.link_id','a.add_time'])
                ->select();
        }elseif($type == 13){
            //首页广告
            ...
            $list = self::where('a.recommend_id', $recommend_id)
                ...
                ->field(['d.id', 'd.title', 'd.image','d.sales','d.ficti','d.money','d.pay_type', 'd.is_show', 'd.member_pay_type', 'd.member_money', 'd.is_del', 'a.link_id','a.add_time'])
                ->select();
        }
        ...
        foreach ($list as &$item) {
            if($type==0 || $type==8){
                ...
                $item['count']=$item['answer'];
            }else if($type==1){
                $item['label'] =json_decode($item['label'],true);
            }else if($type==14){
                $item['image'] = get_oss_process($item['image'],$imagetype);
                $item['special_type_name']='课件资料';
                $item['count']=bcadd($item['sales'],$item['ficti'],0);
            }
        }
        return $list;
    ...
    public static function getRelationCount($recommend_id, $type)
    {
        if ($type == 0){
            ...
            $count = self::where('a.recommend_id', $recommend_id)->alias('a')->join("StoreProduct p", 'p.id=a.link_id')
                ->where(['p.is_del' => 0,'p.is_show' => 1,])->count();
        }else if($type == 5){
            ...
            $count = self::where('a.recommend_id', $recommend_id)->alias('a')->join("__SPECIAL__ p", 'p.id=a.link_id')
                ->join('__SPECIAL_SUBJECT__ j', 'j.id=p.subject_id', 'LEFT')->where(['p.is_show' => 1,'p.is_pink' => 1,'p.is_del' => 0])->count();
        }else if($type == 10){
            ...
            $count = self::where('a.recommend_id', $recommend_id)->alias('a')->join("TestPaper t", 't.id=a.link_id')
                ->where(['t.is_show' => 1,'t.is_del' => 0,'t.type' => 1])->count();
        }else if($type == 12){
            $count = self::where('a.recommend_id', $recommend_id)->alias('a')->join("TestPaper t", 't.id=a.link_id')
                ->where(['t.is_show' => 1,'t.is_del' => 0,'t.type' => 2])->count();
        }else if($type == 13){
