        ...
        try{
            ...
            if($wechat_notification_message==1){
                WechatTemplateService::sendTemplate(WechatUser::where('uid', $order['uid'])->value('openid'), WechatTemplateService::ORDER_USER_SIGN_UP_SUCCESS, [
                    ...
                    'keyword2' => date('Y/m/d/H/i',$event['start_time']).'~'.date('Y/m/d/H/i',$event['end_time']),
                    'keyword3' => $event['province'].$event['city'].$event['district'].$event['detail'],
                    'remark' => '点击查看报名详情'
                ], $site_url . Url::build('/m/my-activity'));
                ...
                RoutineTemplate::sendSignUpSuccess($data,$order['uid'],$site_url . Url::build('/m/my-activity'));
