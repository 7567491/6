...
class StoreOrder extends ModelBasic
{
    ...
    public static function createReceiveGift($orderId, $uid)
    {
        ...
        if (SpecialBuy::be(['special_id' => $order['cart_id'], 'uid' => $uid, 'is_del' => 0])) return self::setErrorInfo('您已拥有此课程，无法进行领取');
        $data = [
            'uid' => $uid,
    ...
    public static function createSpecialOrder($special, $pinkId, $pay_type, $uid, $payType, $link_pay_uid = 0, $total_num = 1)
    {
        ...
        switch ((int)$pay_type) {
            ...
            case 2:
                ...
                if($res) return self::setErrorInfo('您已获得课程，无需再次购买!');
                break;
            case 3:
                //参与拼团
                ...
                $res=SpecialBuy::PaySpecial($special->id,$uid);
                if($res) return self::setErrorInfo('您已获得课程，不能参与拼团!');
    ...
    public static function h5SpecialPay($orderId,$field = 'order_id')
    {
        ...
        $site_name = SystemConfigService::get('site_name');
        if(!$site_name) exception('支付参数缺少：请前往后台设置->系统设置-> 填写 网站名称');
        return WechatService::paymentPrepare(null,$orderInfo['order_id'],$orderInfo['pay_price'],'special',self::getSubstrUTf8($site_name.'-课程购买',30),'','MWEB');
    }
    /**课程余额支付
     * @param $order_id
     * @param $uid
...
//                ], $site_url . Url::build('/m/my-fav'));
                ...
                ], $site_url . Url::build('/m/my-courses'));
                ...
                $data['thing6']['value'] =  '您购买的课程已支付成功！';
                RoutineTemplate::sendOrderSuccess($data,$order['uid'],$site_url . Url::build('/m/my-fav'));
    ...
    public static function payMeSuccess($orderId)
    {
        ...
        try{
            ...
            if($wechat_notification_message==1){
                ...
                RoutineTemplate::sendOrderSuccess($data,$order['uid'],$site_url . Url::build('/m/my-vip'));
                $dataAdmin['character_string1']['value'] =  $orderId;
                $dataAdmin['amount3']['value'] =  $order['pay_price'];
                $dataAdmin['time2']['value'] =  date('Y-m-d H:i:s',time());
    ...
    public static function payGoodsSuccess($orderId)
    {
        ...
        if(!$order) return false;
        $res1 = self::where('order_id', $orderId)->where('type',2)->update(['paid' => 1,'pay_time' => time()]);
        $site_url = SystemConfigService::get('site_url');
        $res2=true;
        try{
            if($res1 && $order['pay_type']!='yue') {
            ...
            if($wechat_notification_message==1){
                ...
                RoutineTemplate::sendOrderSuccess($data,$order['uid'],$site_url . Url::build('/m/my-order'));
                $dataAdmin['character_string1']['value'] =  $orderId;
                $dataAdmin['amount3']['value'] =  $order['pay_price'];
                $dataAdmin['time2']['value'] =  date('Y-m-d H:i:s',time());
    ...
    public static function orderTakeAfter($order)
    {
        ...
        $wechat_notification_message = SystemConfigService::get('wechat_notification_message');
        if($wechat_notification_message==1){
            $thing = implode(',', StoreOrderCartInfo::getProductNameList($order['id']));
            if (mb_strlen($thing, 'utf-8') > 20) {
                $thing = mb_substr($thing, 0, 20, 'utf-8');
            }
            WechatTemplateService::sendTemplate($openid, WechatTemplateService::ORDER_TAKE_SUCCESS, [
                'character_string2' => $order['order_id'],
                'character_string7' => date('Y/m/d H:i', time()),
                'thing4' => $thing,
...
//            return self::setErrorInfo('该订单无法删除!');
    ...
    public static function getOrderSpecialInfo($orderId, $uid)
    {
        ...
        if (!$data['special']) return self::setErrorInfo('课程未查找到');
