...
class StoreProductReply extends ModelBasic
{
    ...
    {
        $data['whole']=self::productValidWhere()->where('product_id',$productId)->count();//全部评价
        $data['praise']=self::productValidWhere()->where('product_id',$productId)->where('product_score','>=',5)->count();//好评评价
        $data['review']=self::productValidWhere()->where('product_id',$productId)->where('product_score','>',1)->where('product_score','<',5)->count();//中评评价
        $data['difference']=self::productValidWhere()->where('product_id',$productId)->where('product_score','=',1)->count();//差评评价
        if($data['whole']>0){
