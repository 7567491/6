...
class Special extends AuthController
{
    ...
    {
        if (!$id) $this->failed('缺少参数,无法访问', $this->mHomeUrl);
        if ($gift_uid && $gift_order_id) {
            if ($gift_uid == $this->uid) $this->failed('您不能领取自己的礼物', Url::build('/m/my-courses'));
            if (!User::get($gift_uid)) $this->failed('赠送礼物的用户不存在', Url::build('/m/my-gift'));
            $order = StoreOrder::where(['is_del' => 0, 'order_id' => $gift_order_id])->find();
            if (!$order) $this->failed('赠送的礼物订单不存在', Url::build('/m/my-gift'));
            if ($order->total_num == $order->gift_count) $this->failed('礼物已被领取完', Url::build('/m/my-courses'));
        }
        $special = SpecialModel::getOneSpecial($this->uid, $id);
        if ($special === false) $this->failed(SpecialModel::getErrorInfo('无法访问'), $this->mHomeUrl);
        ...
        if (!$is_member && $special_money['is_mer_visible'] == 1) $this->failed('课程仅会员可以获得，请充值会员', Url::build('/m/my-vip'));
        ...
        if (isset($special['special'])) {
            ...
            if ($specialinfo['type'] == SPECIAL_LIVE) {
                ...
                if (!$liveInfo) return $this->failed('直播间尚未查到！', $this->mHomeUrl);
                if ($liveInfo->is_del) return $this->failed('直播间已经删除！', $this->mHomeUrl);
            }
        }
        $user_level = !$this->uid ? 0 : $this->userInfo;
        $site_url = SystemConfigService::get('site_url') . Url::build('/m/view-course') . '?id=' . $id . '&spread_uid=' . $this->uid;
        $this->assign($special);
        ...
        $this->assign('comment_switch', SystemConfigService::get('special_comment_switch'));//课程评论开关
    ...
    public function single_details($id = 0, $pinkId = 0, $gift_uid = 0, $gift_order_id = null, $link_pay_uid = 0, $partake = 0, $gift = 0, $link_pay = 0)
    {
        ...
        if (isset($special['special'])) {
            ...
            // 对免流视频做处理
            if ($specialinfo['singleProfile']['video_type'] == 4) {
                $fxdisk_full_path = get_fxdisk_full_path($specialinfo['singleProfile']['link']);
                if (!$fxdisk_full_path['status']) {
                    return JsonService::fail($fxdisk_full_path['msg']);
                }
                $specialinfo['singleProfile']['link'] = $fxdisk_full_path['path'];
                $special['special'] = json_encode($specialinfo);
            }
        }
        $user_level = !$this->uid ? 0 : $this->userInfo;
        $site_url = SystemConfigService::get('site_url') . Url::build('/m/single-course') . '?id=' . $id . '&spread_uid=' . $this->uid;
        $viewing_time=0;
        if($this->uid && $id) {
        ...
        if ($watch) {
            $special['watch'] = json_encode($watch);
        } else {
            $special['watch'] = 'false';
        }
        $this->assign($special);
        ...
        $this->assign('isPink', $isPink);
        $this->assign('isPay', $isPay);
        $this->assign('viewing_time', $viewing_time);
        $this->assign('orderId', $gift_order_id);
        $this->assign('link_pay', (int)$link_pay);
        $this->assign('gift', (int)$gift);
        $this->assign('link_pay_uid', $link_pay_uid);
    ...
    // 免流量获取刷新视频地址
    public function free_single_info($id = 0)
    {
        $special = SpecialModel::getSingleOneSpecial($this->uid, $id);
        if (isset($special['special'])) {
            $specialinfo = $special['special'];
    ...
    public function single_text_detail()
    {
        return $this->fetch();
    }
    ...
    public function single_img_content($id = 0)
    {
        ...
        if($isPay == false && $data['profile']['is_try']){
            unset($data['content']);
        }else if($isPay == false && !$data['profile']['is_try']){
            unset($data['content']);
        }
        $site_url = SystemConfigService::get('site_url') . Url::build('/m/single-course') . '?id=' . $id . '&spread_uid=' . $this->uid;
    ...
    public function userdata($uid)
    {
        ...
        if ($avatar) {
            ...
            $user['avatar'] = '/system/images/user_log.png';
        }
        return $user;
    }
    ...
    public function user_comment_special($special_id = 0)
    {
        ...
        $uid = $this->uid;
        if (SpecialReply::be(['special_id' => $special_id, 'uid' => $uid])) return JsonService::fail('该课程已评价!');
    ...
    {
        if (!$pink_id) $this->failed('缺少订单号', Url::build('/m/pink-order'));
        $this->assign([
            'pink_id' => $pink_id,
            'is_help' => $is_help,
        ]);
     ...
     * @return mixed|void
     */
    public function pink($pink_id = 0, $special_id = 0, $is_help = 0, $orderId = null, $spread_uid=0)
    {
        if (is_null($orderId) && $is_help == 0) {
            ...
            if (StorePink::be(['cid' => $special_id, 'uid' => $this->uid])) {
                ...
                if ($pink && $pink_id == $pink['k_id'] || $pink_id == $pink['id']) {
                    ...
                    return $this->failed('您已参与该课程的拼团，不能多次参与');
                }
            }
        }
        $info = StoreOrder::getOrderSpecialInfo($orderId, $this->uid, $spread_uid);
        if ($info === false) return $this->failed(StoreOrder::getErrorInfo(), Url::build('/m/pink-order'));
        ...
        if (!$special) $this->failed('课程不存在', $this->mHomeUrl);
    ...
    public function groupProjectList()
    {
        ...
        $is_member=isset($this->userInfo['level']) ? $this->userInfo['level'] : 0;
        return JsonService::successful(SpecialModel::getPinkSpecialList($where['page'], $where['limit'], $is_member));
    }
    ...
    public function get_pink_info($pinkId = 0)
    {
        ...
        if (!$pink) return JsonService::fail('参数错误', ['url' => Url::build('/m/my')]);
    ...
    {
        if (!$task_id) $this->failed('无法访问', $this->mHomeUrl);
        Session::set('video_token_' . $task_id, md5(time() . $task_id));
        $tash = SpecialTask::get($task_id);
        if (!$tash) $this->failed('您查看的资源不存在', $this->mHomeUrl);
        if ($tash->is_show == 0) $this->failed('您查看的资源已下架', $this->mHomeUrl);
    ...
    {
        list($special_id, $pay_type_num, $payType, $from, $pinkId, $total_num, $link_pay_uid, $key, $mark,$price_id, $event) = UtilService::PostMore([
            ['special_id', 0],
            ['pay_type_num', -1],
            ['payType', 'weixin'],
            ['from', 'weixin'],
        ...
        switch ($pay_type_num) {
            ...
            case 30://虚拟币充值
                ...
                break;
            case 40: //商品购买
                $this->create_goods_order($special_id, $payType, $key, $mark, $from);
                break;
            case 50: //订单再次支付
    ...
    public function create_special_order($special_id, $pay_type_num, $payType, $pinkId, $total_num, $link_pay_uid, $from = 'weixin')
    {
        ...
        if ($pinkId && $pay_type_num != 1) {
            ...
            if (StorePink::be(['uid' => $this->uid, 'type' => 1, 'cid' => $special_id, 'status' => 1])) return JsonService::status('ORDER_EXIST', '您已参见本课程的拼团,请结束后再进行参团');
            if (SpecialBuy::be(['uid' => $this->uid, 'special_id' => $special_id, 'is_del' => 0])) return JsonService::status('ORDER_EXIST', '您已获得此课程,不能在进行参团!');
            //处理拼团完成
            try {
    ...
    {
        if (is_null($orderId)) $this->failed('缺少订单号,无法进行赠送', Url::build('/m/my-gift'));
        if (!$this->uid) $this->failed('未获取到用户信息！', $this->mHomeUrl);
        $special = StoreOrder::getOrderIdToSpecial($orderId, $this->uid);
        if ($special === false) $this->failed(StoreOrder::getErrorInfo(), Url::build('/m/my-gift'));
        if ($special['is_light']) {
            $site_url = SystemConfigService::get('site_url') . Url::build('/m/single-course') . '?id=' . $special['id'] . '&gift_uid=' . $this->uid . '&gift_order_id=' . $orderId . '&gift=1&spread_uid=' . $this->uid;
        } else {
            $site_url = SystemConfigService::get('site_url') . Url::build('/m/view-course') . '?id=' . $special['id'] . '&gift_uid=' . $this->uid . '&gift_order_id=' . $orderId . '&gift=1&spread_uid=' . $this->uid;
        }
        $special['image'] = str_replace("\\", "/", $special['image']);
        $this->assign([
            'orderId' => $orderId,
            'title' => '赠送礼物',
            'site_url' => $site_url,
    ...
    {
        if (is_null($orderId)) $this->failed('缺少订单号,无法查看领取记录', Url::build('/m/my-gift'));
    ...
    public function poster_show($special_id = 0, $pinkId = 0, $is_help = 0)
    {
        ...
        $spread_uid = StorePink::where('id', $pinkId)->value('uid');
        $site_url = SystemConfigService::get('site_url') . Url::build('special/pink') . '?pink_id=' . $pinkId . '&special_id=' . $special['id'] . '&is_help=1&spread_uid=' . $spread_uid;
     ...
     * 获取课程浏览记录
     */
    public function get_special_record()
    {
        $data = UtilService::GetMore([
            ['page', 1],
            ['limit', 12],
        ...
        $is_member = isset($this->userInfo['level']) ? $this->userInfo['level'] : 0;
        $data['uid'] = $this->uid;
        return JsonService::successful(SpecialModel::get_special_record($data, $is_member));
    }
    ...
    {
        if (!$id) return $this->failed('缺少课程id,无法查看', $this->mHomeUrl);
    ...
    public function getTaskInfo()
    {
        ...
        $special = SpecialModel::PreWhere()->where('id', $data['special_id'])->field('pay_type,member_pay_type')->find();
        ...
        $site_url = SystemConfigService::get('site_url') . Url::build('/m/view-course') . '?id=' . $data['special_id'] . '&spread_uid=' . $this->uid;
        ...
        $array['isPay'] = $isPay;
        if ($array['taskInfo']['video_type'] == 4) {
            $fxdisk_full_path = get_fxdisk_full_path($array['taskInfo']['link']);
            if (!$fxdisk_full_path['status']) {
                return JsonService::fail($fxdisk_full_path['msg']);
            }
    ...
    public function member_recharge()
    {
        ...
        $url = SystemConfigService::get('site_url') . Url::build('/m/my-vip', ['spread_uid' => $this->uid]);
        $this->assign([
            'url' => $url,
    ...
    public function inspect($special_id = 0, $is_light = 0, $type=0)
    {
        ...
        $user_level = !$this->uid ? 0 : $this->userInfo;
        $special = SpecialModel::PreWhere()->where('id', $special_id)->field('pay_type,member_pay_type')->find();
        if ($isPay || $special->pay_type == 0 || ($user_level['level'] > 0 && $special->member_pay_type == 0)) {
            $isPay = true;
        }
     ...
     */
    public function create_goods_order($addressId, $payType, $key, $mark, $from = 'weixin')
    {
        ...
        $order = StoreOrder::cacheKeyCreateOrder($this->uid, $key, $addressId, $payType, $mark);
        $orderId = $order['order_id'];
     ...
     * 我的商品订单
     */
    public function order_store_list($type = '')
    {
    ...
    {
        if (!$uni || !$order = StoreOrder::getUserOrderDetail($this->uid, $uni)) return $this->redirect(Url::build('/m/pink-order'));
        $this->assign([
    ...
    public function question_index($id)
    {
        ...
        $user_level = !$this->uid ? 0 : $this->userInfo;
        $this->assign(['uid' => $this->uid, 'titles' => $title, 'is_member' => isset($user_level['level']) ? $user_level['level'] : 0]);
    ...
    public function data_details($id = 0)
    {
        ...
        if ($data === false) $this->failed(DataDownload::getErrorInfo('无法访问'), $this->mHomeUrl);
        $is_member = isset($this->userInfo['level']) ? $this->userInfo['level'] : 0;
        ...
        $site_url = SystemConfigService::get('site_url') . Url::build('/m/view-virtual') . '?id=' . $id . '&spread_uid=' . $this->uid;
        $this->assign($data);
        $this->assign('site_url', $site_url);
