...
//        if (!$special['poster_image']) $this->failed('您查看的海报不存在', Url::build('spread/special'));
        if($special['is_light']){
            $url = SystemConfigService::get('site_url') . Url::build('/m/single-course') . '?id=' . $special['id'] . '&link_pay_uid=' . $this->uid . '&link_pay=1&spread_uid=' . $this->uid . '#link_pay';
        }else {
            $url = SystemConfigService::get('site_url') . Url::build('/m/view-course') . '?id=' . $special['id'] . '&link_pay_uid=' . $this->uid . '&link_pay=1&spread_uid=' . $this->uid . '#link_pay';
        }
        $this->assign([
            'url' => $url,
            'poster_image' => str_replace("\\", "/", $special['poster_image'])
        ]);
        return $this->fetch();
    }
    ...
    public function save_promoter($spread_uid = 0)
    {
        ...
        if ($this->userInfo['spread_uid']==$spread_uid) return JsonService::fail($spreadUserInfo['nickname'].'已是您的上级，无需再次绑定!');
    ...
    public function become_promoter($spread_uid = 0)
    {
        ...
        $this->assign('home_logo', str_replace("\\", "/", SystemConfigService::get('home_logo')));
        return $this->fetch();
    }
