...
//            }
    ...
    {
        if ($search == '') $this->failed('没有查找相关数据,点我返回上一页', $this->mHomeUrl);
    ...
    {
        if (!$recommend_id) $this->failed('您查看的页面走丢了', $this->mHomeUrl);
        $recommend = Recommend::get($recommend_id);
        if (!$recommend) $this->failed('您查看的栏目不存在', $this->mHomeUrl);
        if ($recommend->is_show == 0) $this->failed('您查看的栏目不存在', $this->mHomeUrl);
