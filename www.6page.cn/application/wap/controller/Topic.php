...
class Topic extends AuthController
{
    ...
    {
        if (!$test_id) $this->failed('缺少参数,无法访问', $this->mHomeUrl);
     ...
     * @throws \think\exception\DbException
     */
    public function viewCertificate($id, $obtain, $uid = 0)
    {
        if($uid) {
            $userInfo = User::getUserInfoCerti($uid);
        } else {
            $userInfo = $this->userInfo;
            $uid = $this->uid;
        }
        if (!$uid) return JsonService::fail('用户不存在');
        $data = CertificateRecord::getCertificate($id,$obtain, $uid);
        if (!$data) return JsonService::fail('证书不存在');
