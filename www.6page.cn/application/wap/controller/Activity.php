...
class Activity extends AuthController
{
    ...
    public function activitySignInList($page = 1, $limit = 20, $navActive = 0)
    {
        ...
        $orderList = $model->order('s.add_time DESC')->field('s.order_id,s.status as statu,s.pay_price,s.activity_id,s.user_info,s.uid,s.code,s.number as upUnmber,r.*')->select();
