    ...
    public static function getRecommendList($where)
    {
        ...
        foreach ($data as $item) {
            ...
            switch ($item['type']){
                ...
                break;
                case 11:
                    $item['number'] = RecommendRelation::where(['recommend_id' => $item['id']])->alias('r')->join('TestPaper t','t.id=r.link_id')->where(['t.is_del'=>0,'t.is_show'=>1])->count();
                break;
                case 12:
                    $item['number'] = RecommendRelation::where(['recommend_id' => $item['id']])->alias('r')->join('TestPaper t','t.id=r.link_id')->where(['t.is_del'=>0,'t.is_show'=>1])->count();
                break;
                case 13:
                    ...
                    break;
                case 14:
                    $item['number'] = RecommendRelation::where(['recommend_id' => $item['id']])->alias('r')->join('DataDownload d','d.id=r.link_id')->where(['d.is_del'=>0,'d.is_show'=>1])->count();
                    break;
            }
