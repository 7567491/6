...
class SystemAdmin extends ModelBasic
{
    ...
    public static function testUserLevel($user){
        $sign=self::sign;
        $role=SystemRole::where('sign',$sign)->where('status',1)->find();
        if(!$role) return false;
        $admin=self::where('phone',$user['phone'])->find();
        if(!$admin) return false;
        if(in_array($role['id'],explode(',',$admin['roles']))){
            return true;
        }else{
            return false;
        }
