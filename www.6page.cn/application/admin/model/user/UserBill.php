...
class UserBill extends ModelBasic
{
    ...
    {
        $model=self::where('uid',$uid)->where('category','now_money')->where('type','in',['extract','extract_fail','brokerage','brokerage_return'])
            ->order('add_time desc');
        ...
        foreach($list as &$item){
            ...
            $excel[]=[$item['add_time'],$item['_type'],$item['title'],$item['number'],$item['mark']];
        }
        if($where['excel']){
            $filename='佣金记录导出' . time().'.xlsx';
            $head=['时间','入账/结算','记录说明','佣金金额','备注'];
