...
use app\admin\model\educational\ContactPhone;
...
class User extends ModelBasic
{
    ...
    public static function setWhere($where)
    {
        ...
        if (isset($where['user_class']) && $where['user_class'] != '') {
            $model = $model->where('u.grade_id', $where['user_class']);
        }
    ...
    public static function getUserList($where)
    {
        ...
        foreach ($list as $k=>&$item){
            ...
            if(!$item['grade_id']){
                ...
                $item['grade_name'] = Classes::where('id', $item['grade_id'])->value('title');
            }
        }
   ...
   */
    ...
    {
        $filename = 'template-user.xlsx'; //获取文件名称
        ...
        if(file_exists(ROOT_PATH.'public' . DS .$filename)){
            header('location:'.$site_url.$filename);exit;
        }else{
    ...
    public static function GetExcelData($filename='1.xlsx',$startLine=4)
    {
        ...
        for($j=$startLine;$j<=(int)$highestRow;$j++){
            ...
            foreach ($width as $key=>$val){
                $v=$getvalue->getCell($val.$j)->getValue();
                if (is_object($v)) {
                    $v = $v->__toString();
                }
                if($v) $value[$key]=$v;
    ...
    public static function importUsers($data=[])
    {
        foreach ($data as $key=>$value){
            if(!isset($value['phone']) || !isset($value['pwd'])) continue;
            $array['account']=$value['phone'];
            $array['phone']=$value['phone'];
            $array['pwd']=md5($value['pwd']);
            $array['nickname']=$value['nickname'] ? $value['nickname'] : $value['phone'];
            $array['avatar']=$value['avatar'] ? $value['avatar'] : '/system/images/user_log.png';
            $array['status']=$value['status'] === '' || intval($value['status']) === 1 ? 1 : 0;
            $array['grade_id']=$value['grade_id'] ? (int)$value['grade_id'] : 0;
            $array['now_money'] = $value['now_money'] ? (float)$value['now_money'] : 0;
            $array['gold_num']= $value['gold_num'] ? (float)$value['gold_num'] : 0;
            $array['full_name']= $value['full_name'] ? $value['full_name'] : '';
            $array['id_card']= $value['id_card'] ? $value['id_card'] : '';
            $array['add_time']=time();

            $array['add_ip'] = '127.0.0.1';
            $array['last_ip'] = '';
            $array['level'] = 0;
            $array['spread_uid'] = 0;
            $array['last_time'] = 0;
            if(self::be(['phone'=>$value['phone']])) continue;
            $res = self::set($array);
            $uid = $res['uid'];
            // 把用户加入班级
            if($array['grade_id']){
                if (!Student::be(['uid'=>$uid,'classes_id'=>$array['grade_id']])) {
                    $student_data = [
                        'uid' => $uid,
                        'classes_id' => $array['grade_id'],
                        'name' => $array['nickname'],
                        'image' => $array['avatar'],
                    ...
                    $student = Student::set($student_data);
                    // 保存联系方式
                    $sid = $student['id'];
                    ContactPhone::set([
                        'sid' => $sid,
