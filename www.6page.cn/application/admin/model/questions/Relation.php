...
class Relation extends ModelBasic
{
    ...
    public static function getSpecialJion($relationship_id,$relationship)
    {
      return  self::alias('r')->join('Special s','r.relation_id=s.id')
                ...
                ->field('s.type,s.title,s.is_light,s.light_type,s.image,s.id,r.sort,r.id as rid,r.relationship,r.relationship_id');
    }
    /**获取试题关联的课程
     * @param $id
     */
     ...
     * @param $relationship
     */
    public static function getRelationTestPaper($id,$relationship)
    {
        $data=self::getTestPaperJion($id,$relationship)->order('r.sort DESC,rid DESC')->select();
        $data=count($data)>0 ? $data->toArray() : [];
     ...
     * @param $relationship
     */
    public static function getRelationDataDownload($id,$relationship)
    {
        $data=self::getDataDownloadJion($id,$relationship)->order('r.sort DESC,rid DESC')->select();
        $data=count($data)>0 ? $data->toArray() : [];
