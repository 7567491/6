...
class Questions extends ModelBasic
{
    ...
    public static function questionsList($where,$arrays=[]){
        ...
        foreach ($data as $key=>&$value){
            ...
            $value['cate']= QuestionsCategpryModel::where(['id'=>$value['pid'],'is_del'=>0])->value('title');
        }
        if (isset($where['excel']) && $where['excel'] == 1) {
            self::SaveExcel($data);
        }
    ...
    public static function SaveExcel($list)
    {
        ...
        $head=['编号', '分类', '题干','配图', '题型', '选项', '答案', '难度', '答案解析', '答题人数','添加时间'];
    ...
    public static function importQuestions($data=[])
    {
        foreach ($data as $key=>$value){
            ...
            switch ($value['question_type']){
                case 1:
                    if($value['a']) array_push($dat,$value['a']);
                    if($value['b']) array_push($dat,$value['b']);
                    if($value['c']) array_push($dat,$value['c']);
                    if($value['d']) array_push($dat,$value['d']);
                break;
                case 2:
                    if($value['a']) array_push($dat,$value['a']);
                    if($value['b']) array_push($dat,$value['b']);
                    if($value['c']) array_push($dat,$value['c']);
                    if($value['d']) array_push($dat,$value['d']);
                    if($value['e']) array_push($dat,$value['e']);
                    if($value['f']) array_push($dat,$value['f']);
                break;
                case 3:
                    if($value['a']) array_push($dat,$value['a']);
                    if($value['b']) array_push($dat,$value['b']);
            ...
            $array['add_time']=time();
            if(self::be(['stem'=>$value['stem'],'is_del'=>0])) continue;
            self::set($array);
        }
        return true;
    }
