...
class TestPaperQuestions extends ModelBasic
{
    ...
    public static function setRecordWhere($test_id,$record_id)
    {
        $model =self::alias('tq')->where('tq.test_id',$test_id)
            ->join('ExaminationTestRecord e','e.questions_id=tq.questions_id and e.e_id='.$record_id,'LEFT')
            ->join('Questions q','q.id=tq.questions_id')->field('q.stem,q.question_type,e.is_correct,e.score');
        return $model;
    }
     ...
     * @param $where
     */
    public static function getExaminationRecordAnswers($where,$test_id,$record_id)
    {
        $model=self::setRecordWhere($test_id,$record_id);
        ...
        $count = self::setRecordWhere($test_id,$record_id)->count();
        return compact('data', 'count');
    }
