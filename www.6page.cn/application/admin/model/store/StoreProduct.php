...
class StoreProduct extends ModelBasic
{
    ...
    public static function storeProductList($where,$special_source)
    {
        ...
        $model=self::getModelObject($where)->where('id','not in',$special_source)->field(['p.*']);
        $model=$model->page((int)$where['page'],(int)$where['limit']);
        $data=($data=$model->select()) && count($data) ? $data->toArray():[];
        $count=self::getModelObject($where)->where('id','not in',$special_source)->count();
    ...
    public static function ProductList($where){
        ...
        foreach ($data as &$item){
            ...
            $item['mp_recommend'] = MpRecommendRelation::where('a.link_id', $item['id'])->where('a.type', 4)->alias('a')
                ->join('__MP_RECOMMEND__ r', 'a.recommend_id=r.id')->column('a.id,r.title');
            $cateName = CategoryModel::where('id',$item['cate_id'])->find();
            $item['cate_name'] = $cateName ? $cateName->cate_name : "";
