                        ...
                        <div class="layui-tab-item layui-show">
                            <div class="layui-form-item">
                                ...
                                <div class="layui-input-block">
                                    <input type="text" name="title" v-model.trim="formData.title" autocomplete="off" placeholder="请输入名称" maxlength="30" class="layui-input">
                                </div>
                            </div>
                            <div class="layui-form-item">
                            ...
                            <div class="layui-form-item">
                                ...
                                <div class="layui-input-block">
                                    <ul class="layui-rate">
                                        <li v-for="item in 5" :key="item" class="layui-inline" @click="onRateClick(item)" @mousemove="onRateMousemove(item)" @mouseleave="onRateMouseleave">
                                            <i :class="[item <= (difficultyHover || formData.difficulty) ? 'layui-icon-rate-solid' : 'layui-icon-rate', 'layui-icon']"></i>
                                        </li>
                                    </ul>
                                </div>
                            </div>
        ...
        new Vue({
            ...
            methods: {
                ...
                questions_task:function (question_type) {
                    var that=this;
                    var url="{:Url('admin/questions.test_paper/questions')}?question_type="+question_type+'&id='+id;
                    layer.open({
                        type: 2 //Page层类型
                        ,area: ['80%', '90%']
                        ,title: '关联试题'
                        ...
                        yes: function(){
                            ...
                            if(question_type==1){
                                var questions_tmp_arr = JSON.parse(questions_tmp);
                                that.single_list.push(...questions_tmp_arr);
                                that.formData.single_tmp_list.push(...questions_tmp_arr);
                                that.single_list = that.single_list.slice(0, that.formData.single_number);
                                that.formData.single_tmp_list = that.formData.single_tmp_list.slice(0, that.formData.single_number);
                                that.show_single_list();
                            }else if(question_type==2){
                                var questions_tmp_arr = JSON.parse(questions_tmp);
                                that.many_list.push(...questions_tmp_arr);
                                that.formData.many_tmp_list.push(...questions_tmp_arr);
                                that.many_list = that.many_list.slice(0, that.formData.many_number);
                                that.formData.many_tmp_list = that.formData.many_tmp_list.slice(0, that.formData.many_number);
                                that.show_many_list();
                            }else{
                                var questions_tmp_arr = JSON.parse(questions_tmp);
                                that.judge_list.push(...questions_tmp_arr);
                                that.formData.judge_tmp_list.push(...questions_tmp_arr);
                                that.judge_list = that.judge_list.slice(0, that.formData.judge_number);
                                that.formData.judge_tmp_list = that.formData.judge_tmp_list.slice(0, that.formData.judge_number);
                ...
                show_single_list:function () {
                    ...
                    table.render({
                        ...
                        ,cols: [[
                            ...
                            {field: 'sort', title: '排序/倒序(以整套试卷排列)',edit:'sort',align: 'center'},
                ...
                save: function () {
                    ...
                    that.$nextTick(function () {
                        if(!that.formData.title) return layList.msg('请输入试卷名称');
                        if(that.formData.tid<=0) return layList.msg('请选择试卷分类');
                        if(that.formData.image=='' && type==2) return layList.msg('请输入试卷封面');
                        if(Number(that.formData.txamination_time)<=0 && type==2) return layList.msg('考试时长填写不正确');
                        if(Number(that.formData.frequency)<0 && type==2) return layList.msg('考试次数填写不正确');
                        
                        if (Number(that.formData.single_number)<0 || Number(that.formData.many_number)<0 || Number(that.formData.judge_number)<0) {
                            return layList.msg('各类型题目数量不能小于0');
                        }
                    ...
                    layList.basePost(layList.U({
                        ...
                        layList.loadClear();
                        if (parseInt(id) == 0) {
                            layList.layer.confirm('添加成功,您要继续添加试卷吗?', {
                                btn: ['继续添加', '立即提交'] //按钮
                            }, function (index) {
