        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-header">
                    <div style="font-weight: bold;">答题记录</div>
                </div>
...
<script>
    ...
    var uid=<?=$uid?>;
    //加载列表
    layList.tableList({o:'List', done:function () {}},"{:Url('getExaminationRecords')}?type=" + type + "&test_id=" + test_id + "&uid=" + uid,function (){
        var join=new Array();
        ...
        switch (event) {
            case 'answers':
                layer.open({
                    type: 2,
                    title: '答题情况',
                    content: "{:Url('answers')}?record_id="+data.id + "&test_id="+data.test_id+"&type="+data.type,
                    area: ['80%', '90%'],
                    maxmin: true
                });
                break;
   ...
   $('.layui-btn').on('click', function () {
       ...
       if (types == 'export') {
          ...
          location.href= layList.U({a:'getExaminationRecords',q:{test_id:test_id,title:title,type:type,excel:1}});
       }
   });
</script>
{/block}
