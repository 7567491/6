                    ...
                    <div class="layui-card-body">
                        <div class="layui-form-item">
                            ...
                            <div class="layui-input-block">
                                <input type="text" name="title" v-model.trim="formData.title" autocomplete="off" placeholder="请输入文章标题" class="layui-input">
                            </div>
                        </div>
                       <div class="layui-form-item cate">
                            <label class="layui-form-label required">文章分类：</label>
                            <div class="layui-input-block">
                        ...
                        <div class="layui-form-item">
                            ...
                            <div class="layui-input-block">
                                <textarea placeholder="请输入文章简介" v-model="formData.synopsis" class="layui-textarea"></textarea>
                            </div>
                        </div>
                        <div class="layui-form-item">
                            <label class="layui-form-label required">文章封面：（500*335）</label>
                            <div class="layui-input-block">
                        ...
                        <div class="layui-form-item cate">
                            <label class="layui-form-label required">文章标签：</label>
        ...
        new Vue({
            ...
            methods:{
                ...
                save:function(){
                    ...
                    if(!that.formData.title) return layList.msg('请输入文章标题');
                    if(that.formData.cid<=0) return layList.msg('请选择文章分类');
                    if(!that.formData.synopsis) return layList.msg('请输入文章简介');
                    if(!that.formData.image_input) return layList.msg('请上传文章封面');
                    if (!that.formData.label.length) return layList.msg('请添加文章标签');
                    if (!that.formData.content) {
                        return layList.msg('请编辑文章内容');
                    }
                    layList.loadFFF();
                    layList.basePost(layList.U({a:'save_article',q:{id:id}}),that.formData,function (res) {
                        layList.loadClear();
                        if(parseInt(id)==0) {
                            layList.layer.confirm('添加成功,您要继续添加文章吗?', {
                                btn: ['继续添加', '立即提交'] //按钮
                            }, function (index) {
