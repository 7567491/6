        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-header">
                    <div style="font-weight: bold;">试卷订单</div>
                </div>
...
{block name="script"}
<script>
    layList.tableList('List',"{:Url('data_download_order_list',['real_name'=>$real_name])}",function (){
        return [
            {field: 'order_id', title: '订单号', width:'13%',templet:'#order_id'},
            {field: 'nickname', title: '用户信息',templet:'#userinfo',align: 'center', width:'10%'},
            {field: 'info', title: '资料信息',templet:"#info"},
    ...
    layList.tool(function (event,data,obj) {
        switch (event) {
            case 'delete':
                var url=layList.U({c:'order.data_download_order',a:'delete',q:{id:data.id}});
    ...
    require(['vue'],function(Vue) {
        new Vue({
            ...
            methods: {
                ...
                getBadge:function() {
                    var that=this;
                    layList.basePost(layList.Url({c:'order.data_download_order',a:'getBadge'}),this.where,function (rem) {
                        that.badge=rem.data;
                    });
                },
                search:function () {
                ...
                excel:function () {
                    this.where.excel=1;
                    location.href=layList.U({c:'order.data_download_order',a:'data_download_order_list',q:this.where});
                }
            },
            mounted:function () {
                var that=this;
