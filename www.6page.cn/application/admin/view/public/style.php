...
<script>
    var img_domain = "{$Think.const.IMG_DOMAIN}";
    function fx_cdn_img(src) {
        if (!src) return;
    ...
    function handle_editor_img(list) {
        for (let i = 0; i < list.length; i++) {
            let item = list[i];
            if (item.src.indexOf('http') !== 0) {
                list[i].src = img_domain + item.src;
                list[i]._src = img_domain + item._src;
            }
        }
        return list;
    }
...
<script>
    requirejs.config({
        ...
        baseUrl:'//'+location.hostname + (location.port ? ':'+location.port : '') +'{__PUBLIC_PATH}',
        paths: {
            'static':'static',
            'system':'system',
            ...
            'layer':'static/plug/layer/layer-admin',
