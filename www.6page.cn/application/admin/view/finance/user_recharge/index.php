        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-header">
                    <div style="font-weight: bold;">虚拟币充值订单</div>
                </div>
                ...
                <div class="layui-card-body">
                    ...
                    <script type="text/html" id="act">
                            ...
                            <ul class="layui-nav-child layui-anim layui-anim-upbit">
                                ...
                                {{#  if(d.price!=d.refund_price){ }}
                                <li>
                                    <a href="javascript:void(0);" onclick="$eb.createModalFrame('退款','{:Url('edit')}?id={{d.id}}',{w:400,h:300})">
...
{block name="script"}
<script>
    layList.tableList('List',"{:Url('get_user_recharge_list',['real_name'=>$real_name])}",function (){
        return [
            {field: 'order_id', title: '订单号', width:'14%',templet:'#order_id'},
            {field: 'nickname', title: '用户信息',templet:'#userinfo',align: 'center', width:'10%'},
            {field: 'price', title: '实际支付',align: 'center', width:'10%'},
            {field: 'gold_num', title: '获得虚拟币数量',align: 'center', width:'10%'},
            {field: 'paid', title: '支付状态',templet:'#paid',align: 'center', width:'10%'},
            {field: 'refund_price', title: '退款金额',align: 'center', width:'10%'},
            {field: 'status', title: '订单状态',templet:'#status',align: 'center', width:'15%'},
            {field: 'pay_time', title: '充值时间',align: 'center', width:'11%'},
            {field: 'right', title: '操作',align:'center',toolbar:'#act', width:'10.2%'},
        ];
    });
    layList.tool(function (event,data,obj) {
        switch (event) {
            case 'delete':
                var url=layList.U({c:'finance.user_recharge',a:'delete',q:{id:data.id}});
    ...
    require(['vue'],function(Vue) {
        new Vue({
            ...
            methods: {
                ...
                getBadge:function() {
                    var that=this;
                    layList.basePost(layList.Url({c:'finance.user_recharge',a:'getBadge'}),this.where,function (rem) {
                        that.badge=rem.data;
                    });
                },
                search:function () {
                ...
                excel:function () {
                    this.where.excel=1;
                    location.href=layList.U({c:'finance.user_recharge',a:'get_user_recharge_list',q:this.where});
                }
            },
            mounted:function () {
                var that=this;
