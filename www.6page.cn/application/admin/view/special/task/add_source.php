...
<style>
    ...
    .prism-player .prism-cc-btn, .prism-player .prism-setting-btn {
        display: none;
    }
</style>
...
<div v-cloak id="app" class="layui-fluid">
    <div class="layui-card">
        <div class="layui-card-body">
            <form action="" class="layui-form" lay-filter="taskForm">
                <div class="layui-form-item submit top-save">
                ...
                <div class="layui-form-item">
                    <label class="layui-form-label required">课节名称：</label>
                    <div class="layui-input-block">
                        <input type="text" name="title" v-model.trim="formData.title" autocomplete="off" placeholder="请输入课节名称" class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item submit">
                    <label class="layui-form-label">课节分类：</label>
                    <div class="layui-input-block">
                ...
                <div class="layui-form-item">
                    <label class="layui-form-label">课节排序：</label>
                ...
                <div class="layui-form-item required">
                    ...
                    <div class="layui-input-inline">
                        <div class="upload-picture-card">
                            <ul v-if="formData.image" class="upload-list">
                                <li class="upload-item">
                                    <img class="upload-thumb" :src="formData.image">
                                    <div class="upload-action">
                                        <i class="fa fa-search-plus" @click="look(formData.image)"></i>
                                        <i class="fa fa-trash" @click="del('image')"></i>
                                    </div>
                                </li>
                            </ul>
                            <div v-else class="upload-image" @click="upload('image')">
                                <div class="fiexd"><i class="fa fa-plus"></i></div>
                                <p>选择图片</p>
                            </div>
                        </div>
                        <div class="layui-form-mid layui-word-aux" style="margin-left: 0;">可选设置，用于将该课节推荐到移动端首页或音频课节顶部显示</div>
                    </div>
                </div>
                ...
                <div class="layui-form-item"  v-show="(source_type==2 || source_type==3) && formData.video_type!=4 && formData.video_type!=1">
                ...
                <div class="layui-form-item"  v-show="source_type==3 || source_type==2">
                ...
                <div class="layui-form-item" v-show="source_type==2">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="title" v-model.trim="link" style="width:50%;display:inline-block;margin-right: 10px;" autocomplete="off" placeholder="请输入音频链接" class="layui-input">
                        ...
                        <button type="button" class="layui-btn layui-btn-normal layui-btn-sm" v-show="is_upload" @click="delVideo()">删除</button>
                        <button v-show="link" type="button" class="layui-btn layui-btn-warm layui-btn-sm" @click="previewSource">预览</button>
                    </div>
                    <div class="layui-input-block" style="width: 50%;margin-top: 20px" v-show="is_video">
                        <div class="layui-progress" style="margin-bottom: 10px">
                            <div class="layui-progress-bar layui-bg-blue" :style="'width:'+videoWidth+'%'"></div>
                    ...
                    <div class="layui-form-mid layui-word-aux" style="margin-left: 0;">可以直接输入音频链接，也可以在此处上传到阿里云点播平台，直接上传部分格式不支持，建议mp3格式</div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">试{{ source_type == 2 ? '听' : '看' }}：</label>
                    <div class="layui-input-block">
                        <input :checked="formData.is_try" type="checkbox" name="is_try" lay-skin="switch" lay-text="开启|关闭" lay-filter="isTry">
                    </div>
                </div>
                <div v-show="formData.is_try && source_type != 1" class="layui-form-item required">
                    <label class="layui-form-label">试{{ source_type == 2 ? '听' : '看' }}时间：</label>
                    <div class="layui-input-inline">
                        <input v-model.number="formData.try_time" type="number" min="1" max="60" class="layui-input">
                    </div>
                ...
                <div v-show="source_type==1" class="layui-form-item required">
                ...
                <div class="layui-form-item required">
                    <label class="layui-form-label">课节简介：</label>
                    <div class="layui-input-block">
                        <textarea id="editor2">{{formData.detail}}</textarea>
                    </div>
                </div>
                <div class="layui-form-item submit">
                    <div class="layui-input-block">
        ...
        new Vue({
            ...
            data: {
                formData:{
                    ...
                    video_type:special.video_type ? special.video_type : 1,
                    file_type:special.file_type ? (special.file_type || '') : '',
                    file_name:special.file_name ? (special.file_name || '') : '',
                    is_try: special.is_try || 0,
                    try_time: special.try_time || 1,
                    try_content: special.try_content || ''
                },
            ...
            methods:{
                ...
                previewSource: function () {
                    if (this.link.indexOf('http')) {
                        ...
                        new Aliplayer({
                            ...
                            source: this.link || '',
                            height: '100%',
                            cover: this.source_type == '2' ? '' : this.formData.image,
                            autoplay: false,
                            format: this.source_type == '2' ? 'mp3' : ''
                        }, function (player) {
                            layList.layer.open({
                                type: 1,
                                title: false,
                ...
                save:function () {
                    var that=this;
                    that.formData.detail = that.editor2.getContent();
                    if(!that.formData.title) return layList.msg('请输入课节名称');
                    // if(!that.formData.image) return layList.msg('请输入课节封面');
                    if (that.source_type != 1) {
                        // if (that.demand_switch == '1') {
                        //     if (that.formData.videoId) {
                        //         that.formData.link = '';
                        //     } else if (that.formData.link) {
                        //         that.formData.videoId = '';
                        //         that.formData.file_type = '';
                        //         that.formData.file_name = '';
                        //     }
                        // } else if (that.demand_switch == '2') {
                        //     if (that.formData.videoId) {
                        //         that.formData.link = '';
                        //     } else {
                        //         that.formData.videoId = '';
                    ...
                    if (this.formData.is_try) {
                        if (that.source_type == 1) {
                            ...
                            if (!that.formData.try_content) {
                                return layList.msg('请编辑试看内容');
                            }
                        } else {
                            if (typeof that.formData.try_time == 'number') {
                                that.formData.try_time = that.formData.try_time.toString();
                            }
                            if (that.formData.try_time) {
                                if (isNaN(that.formData.try_time)) {
                                    return layList.msg('请输入正确的试' + (that.source_type == 2 ? '听' : '看') + '时间');
                                } else {
                                    if (parseInt(that.formData.try_time) == parseFloat(that.formData.try_time)) {
                                        that.formData.try_time = parseInt(that.formData.try_time).toString();
                                        if (that.formData.try_time < 1) {
                                            return layList.msg('请输入不小于1的试' + (that.source_type == 2 ? '听' : '看') + '时间');
                                        } else {
                                            if (that.formData.try_time > 60) {
                                                return layList.msg('请输入不大于60的试' + (that.source_type == 2 ? '听' : '看') + '时间');
                                            }
                                        }
                                    } else {
                                        return layList.msg('请输入整数的试' + (that.source_type == 2 ? '听' : '看') + '时间');
                                    }
                                }
                            } else {
                                return layList.msg('请输入试' + (that.source_type == 2 ? '听' : '看') + '时间');
                            }
                        }
                    }
                    if (that.source_type == 1) {
                        ...
                        if (!that.formData.content) {
                            return layList.msg('请编辑课节内容');
                        }
                    }
                    if (!that.formData.detail) {
                        return layList.msg('请编辑课节简介');
                    }
                    layList.loadFFF();
                    layList.basePost(layList.U({a:'save_source',q:{id:id,special_type:that.source_type}}),that.formData,function (res) {
                        ...
                        if(parseInt(id) == 0) {
                            if (that.source_type == 1) {
                                layList.layer.confirm('添加成功，您要继续添加课节吗？', {
                                    btn: ['继续添加', '立即提交'] //按钮
                                }, function (index) {
            ...
            mounted:function () {
                ...
                if (that.source_type == 2) {
                    that.link = special.link
                }
                if(that.formData.link && that.formData.videoId=='' && that.source_type!=1){
                    that.is_upload=true;
                    that.link = that.formData.link;
                }else if(that.formData.videoId && that.source_type!=1 && that.formData.video_type != 1 && that.formData.video_type != 4){
                    that.is_upload=true;
                    that.playbackAddress(that.formData.videoId);
                }
                if (that.source_type!=1 && (that.formData.video_type == 1 || that.formData.video_type == 4)) {
                    that.is_upload=true;
                    that.link = that.formData.link;
                }
