            ...
            <div class="layui-row layui-col-space15">
                    ...
                    <form class="layui-form layui-form-pane" action="">
                        <div class="layui-form-item">
                            ...
                            <div class="layui-inline">
                                ...
                                <div class="layui-input-inline">
                                    <input type="text" name="special_name" class="layui-input" placeholder="课程名称、编号">
                                </div>
                            </div>
                            <div class="layui-inline">
                ...
                <div class="layui-col-md12">
                    <div class="layui-btn-group">
                        <button type="button" class="layui-btn layui-btn-normal layui-btn-sm" data-type="add" onclick="action.open_add('{:Url('create_false')}','添加虚拟评论')">
                            <i class="layui-icon">&#xe608;</i>添加虚拟评论
                        </button>
                    ...
                    <script type="text/html" id="is_selected">
                        <input type='checkbox' name='id' lay-skin='switch' value="{{d.id}}" lay-text='是|否' lay-filter='is_selected'  {{ d.is_selected == 1 ? 'checked' : '' }}>
                    </script>
                    <script type="text/html" id="is_show">
                        <input type='checkbox' name='id' lay-skin='switch' value="{{d.id}}" lay-text='显示|隐藏' lay-filter='is_show'  {{ d.is_show == 1 ? 'checked' : '' }}>
                    </script>
                    <script type="text/html" id="act">
                        <button type="button" class="layui-btn layui-btn-normal layui-btn-xs" lay-event='reply'>
                            <i class="iconfont icon-bianji"></i>回复
                        </button>
...
<script>
    ...
    layList.tableList('List',"{:Url('getSpecialReplyList')}?special_id="+special_id,function (){
        return [
            ...
            {field: 'title', title: '课程名称', width:'10%',align:'center'},
            ...
            {field: 'is_show', title: '审核', align:'center',width:'6%',toolbar:'#is_show'},
            {field: 'is_selected', title: '加精', align:'center',width:'6%',toolbar:'#is_selected'},
            {field: 'add_time', title: '评论时间',width:'8%',align:'center'},
            {field: 'right', title: '操作',align:'center',width:'10%',toolbar:'#act'}
        ];
    });
    //自定义方法
    var action= {
    ...
    layList.tool(function (event,data,obj) {
        switch (event) {
            case 'delstor':
                var url=layList.U({c:'special.special_reply',a:'delete',q:{id:data.id}});
                ...
                break;
            case 'reply':
                var url =layList.U({c:'special.special_reply',a:'set_reply'}),
                    id=data.id,
                ...
                break;
            case 'refining':
                var url=layList.U({c:'special.special_reply',a:'refining_reply',q:{id:data.id}});
                $eb.$swal('delete',function(){
