            ...
            <div class="layui-card">
                ...
                <div class="layui-card-body">
                    <div class="layui-row layui-col-space15">
                                ...
                                <div class="layui-form-item">
                                    <div class="layui-inline">
                                        ...
                                        <div class="layui-input-inline">
                                            <input type="text" name="store_name" class="layui-input" placeholder="课程名称、简介、编号">
                        ...
                        <div class="layui-col-md12">
                            <div class="layui-btn-group">
                                <button type="button" class="layui-btn layui-btn-normal layui-btn-sm" data-type="add" onclick="action.open_add('{:Url('single_add',['special_type' =>$special_type])}','添加{$special_title}')">
                                ...
                                <button type="button" class="layui-btn layui-btn-normal layui-btn-sm" data-type="deleteAll" onclick="action.del_all()">
                                    批量删除
                                </button>
                            </div>
                            ...
                            <script type="text/html" id="recommend">
                                ...
                                <div class="layui-btn-container">
                                    {{#  layui.each(d.mp_recommend, function(index, item){ }}
                                    <button type="button" class="layui-btn layui-btn-mp layui-btn-normal layui-btn-xs" data-type="recommend" data-id="{{index}}" data-pid="{{d.id}}">mp-{{item}}</button>
                                    {{#  }); }}
                                </div>
                            </script>
                            ...
                            <script type="text/html" id="act">
                                ...
                                <ul class="layui-nav-child layui-anim layui-anim-upbit">
                                    <li>
                                        <a  href="javascript:void(0)" onclick="action.open_add('{:Url('single_add')}?id={{d.id}}&special_type={$special_type}','编辑课程')">
                                            <i class="iconfont icon-bianji"></i> 编辑课程
                                        </a>
                                    </li>
                                    <li>
                                    ...
                                    <li>
                                        <a href="javascript:void(0)" onclick="$eb.createModalFrame('{{d.title}}-推荐管理','{:Url('mp_recommend')}?special_id={{d.id}}',{h:300,w:400})">
                                            <i class="iconfont icon-yidongshouye"></i> 推至小程序首页
                                        </a>
                                    </li>
                                    <li>
                                        <a href="javascript:void(0)" onclick="$eb.createModalFrame('{{d.title}}-推荐管理','{:Url('web_recommend')}?special_id={{d.id}}&type=0',{h:300,w:400})">
                                    ...
                                    <li>
                                        <a lay-event='delect' href="javascript:void(0)">
                                            <i class="iconfont icon-shanchu"></i> 删除课程
                                        </a>
                                    </li>
                                </ul>
                            </script>
...
<script>
    ...
    //实例化form
    layList.form.render();
    layList.date({elem: '#datetime', theme: '#10952a', type: 'datetime', range: '~'});
    //加载列表
    layList.tableList({o:'List', done:function () {
        ...
        $('.layui-btn-pc').on('click', function (event) {
            ...
            if ('recommend' === type) {
                var id = target.dataset.id;
                var pid = target.dataset.pid;
                var url = layList.U({ a: 'cancel_web_recommendation', q: { id: id, special_id: pid } });
                $eb.$swal(
                    'delete',
                    function () {
                        $eb.axios
            ...
            $('.layui-btn-mp').on('click', function (event) {
                var target = event.target;
                var type = target.dataset.type;
                if ('recommend' === type) {
                    var id = target.dataset.id;
                    var pid = target.dataset.pid;
                    var url = layList.U({ a: 'cancel_mp_recommendation', q: { id: id, special_id: pid } });
                    $eb.$swal(
                        'delete',
                        function () {
                            $eb.axios
        ...
        return [
            {type: 'checkbox'},
            {field: 'id', title: 'ID', width:'7%',align: 'center'},
            {field: 'title', title: '名称'},
            {field: 'subject_name', title: '分类',align: 'center',width:'7%'},
            {field: 'image', title: '封面',templet:'#image',align: 'center',width:'7%'},
            {field: 'recommend', title: '推荐[yd:移动端,mp:小程序,pc:PC端]',templet:'#recommend',align: 'center'},
            {field: 'money', title: '价格',align: 'center',width:'7%'},
            {field: 'member_money', title: '会员价',align: 'center',width:'7%'},
            {field: 'pink_money', title: '拼团价',align: 'center',width:'7%'},
            {field: 'fake_sales', title: '虚拟人数',event:'fake_sales',edit:'fake_sales',align: 'center',width:'7%'},
            {field: 'is_pink', title: '拼团状态',templet:'#is_pink',align: 'center',width:'7%'},
            {field: 'sort', title: '排序',sort: true,event:'sort',edit:'sort',align: 'center',width:'7%'},
            {field: 'is_show', title: '状态',templet:'#is_show',align: 'center',width:'8%'},
    ...
    var action= {
        ...
        del_all: function () {
            ...
            if (ids.length == 0) {
                $eb.$swal('error', '请先选择要删除的课程');
                return
            }
            ids = ids.join(',');
            var url=layList.U({a:'delete',q:{id: ids, model_type:'special'}});
    ...
    layList.tool(function (event,data,obj) {
        switch (event) {
            ...
            case 'learning_records':
                ...
                break;
            case 'comments':
                location.href=layList.U({c:'special.special_reply',a:'index',q:{
                        special_id:data.id
                    }});
                break;
        }
