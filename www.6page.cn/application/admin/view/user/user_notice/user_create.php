...
<style type="text/css">
    ...
    .form-add .sub-btn{border-radius: 0;width: 49.5%;padding: 6px 0;font-size: 14px;outline: none;border: none;color: #fff;background-color: #20a53a;}
    .form-add .sub-btn:nth-child(1){float: left;}
    .form-add .sub-btn:nth-child(2){float: right;}
</style>
<div class="row">
    <div class="col-sm-12">
        <div class="ibox">
...
<div class="form-add">
    <button type="submit" class="sub-btn">提交选中</button>
    <button type="submit" class="sub-btn" sub-type="all">提交全部</button>
</div>
<script>
    $('.i-checks').iCheck({
    ...
    $(".sub-btn").on("click",function(){
        ...
        if($(this).attr("sub-type") == "all"){
            formData.search = {};
            formData.search.data = "{$where.data}";
            formData.search.nickname = "{$where.nickname}";
        }
