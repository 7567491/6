                ...
                <div class="layui-row layui-col-space15">
                            ...
                            <div class="layui-form-item">
                               ...
                               <div class="layui-inline">
                                ...
                                <div class="layui-inline">
                                    ...
                                    <div class="layui-input-inline">
                                        <select name="user_class" lay-verify="user_class">
                                            <option value="">全部</option>
                    ...
                    <div class="layui-col-md12">
                        <div class="layui-btn-group conrelTable">
                            ...
                            <button class="layui-btn layui-btn-normal layui-btn-sm" type="button" data-type="add_user">
                                添加用户
                            </button>
                            <button type="button" class="layui-btn layui-btn-normal layui-btn-sm" data-type="download" >
                                下载导入模版
                            </button>
                            <button class="layui-btn layui-btn-normal layui-btn-sm" type="button" data-type="import">
                                导入用户
                            </button>
                            <button class="layui-btn layui-btn-normal layui-btn-sm" type="button" data-type="delete">
                                批量删除
                            </button>
                        ...
                        <script type="text/html" id="nickname">
                            <a href="javascript:;" onclick="$eb.createModalFrame('用户资料','{:Url('user_data')}?uid='+{{d.uid}},{w:1000})" style="color:#10952a">{{d.nickname ? d.nickname :'暂无昵称'}}</a>
                        </script>
                        ...
                        <script type="text/html" id="barDemo">
                            ...
                            <ul class="layui-nav-child layui-anim layui-anim-upbit">
                                ...
                                <li>
                                    <a href="javascript:void(0);" onclick="$eb.createModalFrame('赠送课程','{:Url('user_data')}?uid='+{{d.uid}},{w:1000})">
                                        <i class="fa fa-paste"></i>赠送课程
                                    </a>
                                </li>
                                <li>
                                    <a href="javascript:void(0);" onclick="$eb.createModalFrame('赠送会员','{:Url('members')}?uid='+{{d.uid}},{w:500,h:400})">
                                ...
                                <li>
                                    <a href="javascript:void(0);" onclick="$eb.createModalFrame('赠送练习','{:Url('send_test')}?uid='+{{d.uid}},{w:1000})">
                                        <i class="fa fa-paste"></i>赠送练习
                                    </a>
                                </li>
                            </ul>
                        </script>
...
<script>
    ...
    layList.tableList('userList',"{:Url('get_user_list')}",function () {
        return [
            ...
            {field: 'full_name', title: '真实姓名',align: 'center'},
            {field: 'title', title: '班级',align: 'center', templet: '<span>{{d.grade_name ? d.grade_name : "未分配"}}</span>'},
            {field: 'levelType', title: '身份',align: 'center'},
            ...
            {field: 'member', title: '会员记录',templet:'#member',align: 'center'},
    ...
    layList.tool(function (event,data) {
        ...
        switch (layEvent){
            case 'edit':
                ...
                break;
            case 'see':
                $eb.createModalFrame(data.nickname+'-用户详情',layList.Url({a:'see',p:{uid:data.uid}}),{w:800});
                break;
            case 'del_vip':
    ...
    var action={
        ...
        set_custom:function () {
            ...
            if(ids.length){
                ...
                layList.msg('请选择要发送客服图文消息的用户');
            }
        },
        add_superior:function () {
            var ids=layList.getCheckData().getIds('uid');
            if(ids.length>0){
                var str = ids.join(',');
                $eb.createModalFrame('设置用户的推广人',layList.Url({c:'user.user',a:'add_superior',p:{uid:str}}),{'w':600,'h':200});
            }else{
                layList.msg('请正确选择要设置的用户');
            }
        },
        add_user:function () {
            $eb.createModalFrame('手动添加用户',layList.Url({c:'user.user',a:'add_user_form',p:{}}),{'w':600,'h':600});
        },
        download:function () {
            location.href= layList.U({a:'downloadExcel'});
        },
        import:function () {
            $eb.createModalFrame('导入用户','{:Url('imports')}',{w:800,h:200})
        },
        delete:function () {
            layer.confirm('删除后用户的学习记录等数据无法恢复！确定要删除选择的所有用户？', {
              title: "谨慎操作",
              icon: 0,
              btn: ['确定', '取消']
                ...
                if(ids.length){
                    var ids = ids.join(',');
                    layList.basePost(layList.Url({a:'deleteAll'}),{uids: ids},function (res) {
                        layList.msg(res.msg);
                        layList.reload();
                    });
                }else{
                    layList.msg('请选择要删除的用户');
                }
            }, function(index){
            })
        },
        add_teacher:function () {
            ...
            if(ids.length==1){
                var str = ids.join(',');
                $eb.createModalFrame('添加老师',layList.Url({c:'educational.teacher',a:'create',p:{uid:str}}),{'w':1200});
            }else{
        ...
        add_student:function () {
            ...
            if(ids.length==1){
                var str = ids.join(',');
                $eb.createModalFrame('添加学员',layList.Url({c:'educational.student',a:'create',p:{uid:str}}),{'w':1200});
            }else{
                layList.msg('请正确选择要成为学员的用户');
            }
        },
        refresh:function () {
            layList.reload();
