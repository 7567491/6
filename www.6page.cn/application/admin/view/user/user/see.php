                            ...
                            <div class="layui-tab-item">
                                <table class="layui-table" v-cloak="">
                                    ...
                                    <tr v-for="(item,index) in PaperList">
                                        <td class="text-center">{{item.order_id}}</td>
                                        <td>{{item.title}}</td>
                                        <td><button class="layui-btn layui-btn-danger layui-btn-xs" @click="del_paper(item,index)">删除</button></td>
                                    </tr>
                                    <tr v-show="PayList.length<=0" style="text-align: center">
                                        <td colspan="6">暂无数据</td>
                                    </tr>
                                ...
                                <div ref="paper_buy" v-show="count.paper_count > limit" style="text-align: right;"></div>
                            </div>
                            <div class="layui-tab-item">
                                <table class="layui-table" v-cloak="">
                                    <thead>
                                    ...
                                    <tr v-for="(item,index) in TestList">
                                        <td class="text-center">{{item.order_id}}</td>
                                        <td>{{item.title}}</td>
                                        <td><button class="layui-btn layui-btn-danger layui-btn-xs" @click="del_test(item,index)">删除</button></td>
                                    </tr>
                                    <tr v-show="PayList.length<=0" style="text-align: center">
                                        <td colspan="6">暂无数据</td>
                                    </tr>
                                ...
                                <div ref="test_buy" v-show="count.test_count > limit" style="text-align: right;"></div>
                            </div>
                            <div class="layui-tab-item">
                                <table class="layui-table" v-cloak="">
                                    <thead>
    ...
    require(['vue'],function(Vue) {
        new Vue({
            ...
            methods:{
                ...
                del_paper:function(item,index){
                    var that=this;
                    layList.layer.confirm('确认要删除此条开通记录？', {
                        btn: ['确认','取消'] //按钮
                    }, function(){
                        layList.baseGet(layList.U({a:"del_paper_buy",q:{id:item.id}}),function (res) {
                            that.PaperList.splice(index,1);
                            that.$set(that,'PaperList',that.PaperList);
                            layList.msg(res.msg);
                        });
                    }, function(){
                        layList.msg('已取消');
                ...
                del_test:function(item,index){
                    ...
                    layList.layer.confirm('确认要删除此条开通记录？', {
                        ...
                        layList.baseGet(layList.U({a:"del_paper_buy",q:{id:item.id}}),function (res) {
                            ...
                            that.$set(that,'TestList',that.TestList);
                            layList.msg(res.msg);
                        });
                    }, function(){
                        layList.msg('已取消');
                ...
                getPaperList:function(){
                    this.request('getPaperList',this.page.pay_page,'PaperList');
                },
                getTestList:function(){
                    this.request('getTestList',this.page.pay_page,'TestList');
                },
            ...
            mounted:function () {
                ...
                layList.laypage.render({
                    elem: that.$refs.paper_buy
                    ,count:that.count.paper_count
                    ,limit:that.limit
                    ,theme: '#10952a',
                    jump:function(obj){
                        that.page.paper_page=obj.curr;
                    }
                });
                layList.laypage.render({
                    elem: that.$refs.test_buy
