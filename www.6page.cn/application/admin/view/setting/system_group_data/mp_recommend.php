            ...
            <div class="layui-card">
                ...
                <div class="layui-card-body">
                    <div class="layui-btn-group">
                        <button type="button" class="layui-btn layui-btn-normal layui-btn-sm" onclick="$eb.createModalFrame('添加推荐分组','{:Url('create_mp_recommend_v1')}',{h:580})">
                    ...
                    <script type="text/html" id="act">
                        ...
                        <ul class="layui-nav-child layui-anim layui-anim-upbit">
                            <li>
                                <div onclick="$eb.createModalFrame('{{d.title}}-'+this.innerText,'{:Url('create_mp_recommend_v1')}?id={{d.id}}',{h:480})">
                            ...
                            {{# if(d.typesetting!=5){ }}
                                ...
                                <li>
                                    <a href="javascript:;" onclick="$eb.createModalFrame('{{d.title}}-'+this.innerText,'{:Url('mp_recommend_article_content')}?id={{d.id}}')">
                                        <i class="fa fa-list-ul"></i> 内容管理
                                    </a>
                                </li>
                                {{# }else{ }}
                                <li>
                                    <a href="javascript:;" onclick="$eb.createModalFrame('{{d.title}}-'+this.innerText,'{:Url('mp_recommend_content')}?id={{d.id}}')">
                                        <i class="fa fa-list-ul"></i> 内容管理
                                    </a>
                                </li>
                                {{# } }}
...
<script>
    ...
    layList.form.render();
    //加载列表
    layList.tableList('List',"{:Url('mp_recommend_list')}",function (){
        return [
    ...
    var action= {
        set_value: function (field, id, value) {
            layList.baseGet(layList.Url({
                a: 'set_value',
                q: {field: field, id: id, value: value,recommend:'mp'}
            }), function (res) {
                layList.msg(res.msg);
            });
        },
    ...
    layList.switch('is_show',function (odj,value) {
        if(odj.elem.checked==true){
            layList.baseGet(layList.Url({a:'set_show',p:{is_show:1,id:value,recommend:'mp'}}),function (res) {
                layList.msg(res.msg);
            });
        }else{
            layList.baseGet(layList.Url({a:'set_show',p:{is_show:0,id:value,recommend:'mp'}}),function (res) {
                layList.msg(res.msg);
            });
        }
    });
    ...
    layList.tool(function (event,data,obj) {
        switch (event) {
            case 'delete':
                var url=layList.U({a:'delete_mp_recomm',q:{id:data.id}});
