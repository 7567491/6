...
<style>
    ...
    .layui-bg-blue {
        background-color: #10952a !important;
    }
                ...
                <div class="layui-card-body">
                    ...
                    <script type="text/html" id="act">
                        ...
                        <ul class="layui-nav-child layui-anim layui-anim-upbit">
                            {{# if(d.type == 3) { }}
                            <li>
                                <div onclick="$eb.createModalFrame('{{d.title}}-'+this.innerText,'{:Url('create_mp_recommend_custom')}?id={{d.id}}&is_fixed=1')">
                                    <i class="fa fa-paste"></i> 编辑
                                </div>
                            </li>
                            {{# }else{ }}
                            <li>
                                <div onclick="$eb.createModalFrame('{{d.title}}-'+this.innerText,'{:Url('create_mp_recommend')}?id={{d.id}}',{h:480})">
                                    <i class="fa fa-paste"></i> 编辑
                                </div>
                            </li>
                            {{# } }}
...
<script>
  ...
  element.on('nav(nav)', function(elem){
    ...
    if (type == 1) {
        $eb.createModalFrame('添加固定跳转导航','{:Url('create_mp_recommend')}',{w:800,h:500});
    } else if (type == 2) {
        $eb.createModalFrame('添加自定义跳转导航','{:Url('create_mp_recommend_custom')}?is_fixed=1');
    }
  });
    //实例化form
    layList.form.render();
    ...
    //加载列表
    layList.tableList('List',"{:Url('mp_recommend_list',['is_fixed' => 1])}",function (){
        return [
