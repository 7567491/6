...
<script>
    ...
    layList.form.render();
    //加载列表
    layList.tableList({o:'List', done:function () {}},"{:Url('getUserTestPaperList')}?type={$type}&uid="+uid,function (){
        return [
        ...
        switch (event) {
            ...
            case 'answers':
                layer.open({
                    type: 2,
                    title: '查看答题',
                    content: "{:Url('admin/questions.test_paper/answerNotes')}?type="+data.type + "&test_id="+data.id + "&uid="+uid,
                    area: ['95%', '95%'],
                    maxmin: true
                });
                break;
