                    ...
                    <div class="layui-form layui-form-pane">
                        <div class="layui-form-item">
                            ...
                            <div class="layui-inline">
                                <input class="layui-input" name="title" id="demoReload" placeholder="请输入课程名称">
                            </div>
                            <div class="layui-inline">
...
<script>
    ...
    var id={$id},relation_ids="{$relation_ids}";
    ...
    table.render({
        elem: '#List'
        ,url:"{:Url('admin/special.special_type/source_list')}?special_id=0&special_type=4"
        ,cols: [[
            {type: 'checkbox'},
        ...
        ,done:function (res,curr,count) {
            ...
            for(var i=0;i< res.data.length;i++){
                ...
                if(relation_ids.length>0){
                    for (var k = 0; k < relation_ids.length; k++) {
                        if(res.data[i].id == relation_ids[k]) {
                            res.data[i]["LAY_CHECKED"]='true';/*设置勾选*/
                            /*找到对应数据改变勾选样式*/
                            var index= res.data[i]['LAY_TABLE_INDEX'];
                            $('tr[data-index=' + index + '] input[type="checkbox"]').prop('checked', true);
            ...
            $("#check_relation_tmp",window.parent.document).val(JSON.stringify(ids));
        }
    });
    var $ = layui.$, active = {
        reload: function(){
