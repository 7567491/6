...
class Student extends AuthController
{
     ...
     * 获取学员列表
     */
    public function getStudentList($cid=0)
    {
        ...
        if($cid>0 && $where['cid']==0) $where['cid']=$cid;
    ...
    public function save_add($id=0){
        ...
        if($id){
            ...
            User::where('uid',$data['uid'])->update(['grade_id'=>$data['classes_id']]);
        }else{
            $user=User::where('uid',$data['uid'])->field('identitys,nickname')->find();
            User::where('uid',$data['uid'])->update(['grade_id'=>$data['classes_id']]);
            if($user['identitys']>0) return Json::fail('该用户已是老师或学员');
            $data['nickname']=$user['nickname'];
            $data['add_time']=time();
            $res=true;
            if(!StudentModel::be(['uid'=>$data['uid'],'is_del'=>0])){
                ...
                $res1=ContactPhoneModel::contactPhoneAdd($id,$contact);
                if($id && $res1) User::where('uid',$data['uid'])->update(['identitys'=>1]);
            }else{
                return Json::fail('该用户已是学员');
            }
        }
        if($res && $id && $res1){
    ...
    public function delete($id=0)
    {
        ...
        if($res){
            User::where('uid',$student['uid'])->update(['identitys'=>0]);
     ...
     *练习列表
     */
    public function getTestPaperListTest()
    {
        $where = parent::getMore([
            ['page', 1],
            ['limit', 20],
     ...
     *移除已发送的练习
     */
    public function removeTestPaperTest($uid,$test_id)
    {
        if(!$uid || !$test_id) return Json::fail('参数错误');
        $res= TestPaperObtain::where(['uid'=>$uid,'test_id'=>$test_id])->update(['is_del'=>1]);
        if($res){
    ...
    public function save_give($uid,$special_id)
    {
        ...
        if (SpecialBuy::be(['uid' => $uid, 'special_id' => $special_id, 'is_del' => 0, 'type' => 3])) return false;
     ...
     * @return mixed
     */
    public function test_paper($uid=0, $type = 2)
    {
        $this->assign(['uid' => $uid, 'type' => $type]);
        return $this->fetch();
    }
    ...
    public function deleteAll(Request $request)
    {
        ...
        $res = StudentModel::where('uid','in', $data['uids'])->delete();
        $existingUids = User::where('uid','in', $data['uids'])->column('uid');
        $res2 = true;
        if (count($existingUids)) {
            $res2 = User::where('uid','in', $existingUids)->update(['grade_id' => 0]);
        }
