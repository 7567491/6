...
use app\admin\model\educational\Student;
...
use app\admin\model\user\PhoneUser;
...
class User extends AuthController
{
    ...
    {
        $classes = Classes::getClassesLists(['page' => 1, 'limit' => 10000, 'status' => '', 'title' => '']);
        ...
        $this->assign('classes', $classes['data']);
        if (!$this->bc) {
     ...
     * @return mixed
     */
    public function send_test($uid=0)
    {
        $this->assign(['uid'=>$uid]);
        return $this->fetch();
    }
     ...
     * @param $tid
     */
    public function sendTestPaperTest($uid,$tid)
    {
        $tid=explode(',',$tid);
        $res= TestPaperObtain::addUidSend($uid,$tid);
        if($res){
    ...
    public function save_give()
    {
        ...
        if (!$post['uid']) return Json::fail('请选择被赠送的用户');
        if (!$post['special_id']) return Json::fail('请选择要赠送的课程');
        $special = Special::get($post['special_id']);
        if (SpecialBuy::be(['uid' => $post['uid'], 'special_id' => $post['special_id'], 'is_del' => 0, 'type' => 3])) return Json::fail('此用户已经拥有此课程无需赠送');
            ...
            if ($special_source){
                foreach($special_source as $k => $v) {
                    ...
                    if ($task_special['is_show'] == 1){
                        SpecialBuy::setBuySpecial('', $post['uid'], $v['source_id'], 3,$task_special['validity'],$post['special_id']);
                    }
                }
            }
        }
        $res=SpecialBuy::setBuySpecial('', $post['uid'], $post['special_id'], 3,$special['validity']);
        // 赠送关联的考试
        \app\web\model\topic\TestPaperObtain::setTestPaper('zs'.time(), $post['uid'], $post['special_id'],3);
        if ($res)
            return Json::successful('赠送成功');
        else
            return Json::fail('赠送失败');
...
//        $f[] = Form::input('account', '用户账号', $user->getData('account'));
        $f[] = Form::input('nickname', '用户昵称', $user->getData('nickname'));
        $f[] = Form::input('full_name', '真实姓名', $user->getData('full_name'));
        $f[] = Form::input('pwd', '修改密码', '');
...
//        if($data['account']) {
//            // 判断account是否重复
//            $accountUser = UserModel::where('account', $data['account'])->find();
//            if($accountUser && $accountUser['uid'] != $uid) return Json::fail('账号已存在');
//            $edit['account'] = $data['account'];
//        }
        if($data['pwd']) {
            $edit['pwd'] = md5($data['pwd']);
            $phoneEdit['pwd'] = md5($data['pwd']);
        }
        if (!empty($phoneEdit)) {
            $pres = PhoneUser::edit($phoneEdit, $uid, 'uid');
            if (!$pres) Json::fail('修改失败');
        }
    ...
    public function getPaperList($uid, $page = 1, $limit = 20)
    {
        // 只查考试
        $type = 2;
        $list=TestPaperObtain::alias('b')->join('TestPaper t','b.test_id=t.id')
            ->where(['b.type'=>$type,'b.uid'=>$uid,'b.is_del'=>0,'t.is_del'=>0,'t.is_show'=>1])
            ->field('b.id,b.uid,b.test_id,b.type,b.source,b.number,b.add_time,b.order_id,t.title,t.image,t.item_number')
            ->page((int)$page,(int)$limit)->order('b.add_time DESC')
            ->select();
        $list=count($list) > 0 ? $list->toArray() : [];
        $count=TestPaperObtain::alias('b')->join('TestPaper t','b.test_id=t.id')
            ->where(['b.type'=>$type,'b.uid'=>$uid,'b.is_del'=>0,'t.is_del'=>0,'t.is_show'=>1])->count();
        foreach ($list as &$value){
            if($value['source']==2) {
                $value['order_id']='领取获得';
    ...
    public function del_paper_buy($id = 0)
    {
        if ($id == 0) return Json::fail('缺少参数');
        $specialBuy = TestPaperObtain::get($id);
        if(!$specialBuy) return Json::fail('记录不存在');
        $res=TestPaperObtain::where('id', $id)->update(['is_del' => 1]);
        $res2 = TestPaperOrder::where('order_id', $specialBuy['order_id'])->update(['is_del' => 1]);
        if ($res)
            return Json::successful('删除成功');
        else
            return Json::fail('删除失败');
    ...
    public function getTestList($uid, $page = 1, $limit = 20)
    {
        // 只查练习
        $type = 1;
        $list=TestPaperObtain::alias('b')->join('TestPaper t','b.test_id=t.id')
    ...
    {
        $f = array();
        $f[] = Form::input('phone', '手机号', '')->required('手机号必填');
        $f[] = Form::input('pwd', '密码', '')->required('必须输入密码');
        $f[] = Form::input('nickname', '昵称', '');
        $f[] = Form::input('full_name', '真实姓名', '');
        $f[] = Form::number('now_money', '余额')->min(0);
        $f[] = Form::radio('is_promoter', '是否推广员', 0) -> options([
            ['value' => 0, 'label' => '关闭'],
            ['value' => 1, 'label' => '推广员'],
        ]);
        $form = Form::make_post_form('添加用户通知', $f, Url::build('add_user'));
        $this->assign(compact('form'));
        return $this->fetch('public/form-builder');
    }
    /**
    ...
    public function add_user(Request $request)
    {
        ...
        // 账号和手机号一致
        $data['account'] = $request->post('phone');
        if (!$data['nickname']) {
        ...
        $phone_user = PhoneUser::create($data);
        if ($phone_user->id) return Json::successful('添加成功!');
    ...
    public function importUsers()
    {
        ...
        $data=UserModel::GetExcelData($where['link'],4);
        if(count($data)<=0) return Json::fail('导入文件内容为空');
        $res=UserModel::importUsers($data);
        if ($res)
    ...
    public function deleteAll(Request $request)
    {
        ...
        $res = UserModel::where('uid','in', $data['uids'])->delete();
        $existingUids = Student::where('uid','in', $data['uids'])->column('uid');
        $res2 = true;
        if (count($existingUids)) {
            $res2 = Student::where('uid','in', $existingUids)->delete();
        }
