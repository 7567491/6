...
class AliyunLive extends AuthController
{
    ...
    // 停止直播
    public function stop_live($id = 0)
    {
        if (!$id) return Json::fail('缺少参数');
        $liveInfo = LiveStudio::get($id);
        if (!$liveInfo) return Json::fail('未查到直播间');
        // 获取当前日期 时分秒
        $date = date('Y-m-d H:i:s');
        $res = LiveStudio::edit(['stop_play_time' => $date, 'is_play' => 0, 'recording' => 0], $id);
        if ($res)
    ...
    public function special_live($subject_id = 0)
    {
        ...
        if (!$special_type || !is_numeric($special_type)) return $this->failed('课程类型参数缺失');
    ...
    public function go_live($id = 0)
    {
        ...
        if (!$specialInfo) return Json::fail('直播课程暂未查到');
        if ($specialInfo->type!=SPECIAL_LIVE) return Json::fail('此课程不授予直播课程，无法获取推流地址！');
    ...
    public function send_remind($id = 0)
    {
        if (!$id) return Json::fail('缺少参数');
        $specialInfo = SpecialModel::get($id);
        if (!$specialInfo) return Json::fail('直播课程暂未查到，无法进行提醒');
        $liveInfo = LiveStudio::where('special_id', $specialInfo->id)->find();
               ...
               if ($openId = WechatUser::where('uid',$v)->value('openid')) {
                   ...
                   if($wechat_notification_message==1){
                       WechatTemplateService::sendTemplate($openId,WechatTemplateService::LIVE_START_NOTICE, [
                           ...
                           'remark' => '直播间通道'
                       ], $site_url . Url::build('/m/view-course',['id' => $id]));
                   }else{
                       $data['thing5']['value'] =  $specialInfo->title;
                       $data['time2']['value'] =  $liveInfo->start_play_time;
                       RoutineTemplate::sendBroadcastReminder($data,$v,$site_url . Url::build('/m/view-course',['id' => $id]));
                   }
               }
    ...
    public function delete($id = 0)
    {
        ...
        foreach ($id_arr as $k => $id_item) {
            $specialInfo = SpecialModel::get($id_item);
            if (!$specialInfo) return Json::fail('查找的课程不存在');
            if ($specialInfo->is_del) return Json::fail('课程已经删除');
            $liveInfo = LiveStudio::where(['special_id' => $id_item])->find();
            if (!$liveInfo) return Json::fail('删除的课程直播不存在');
            if ($liveInfo->is_del) return Json::fail('直播间已删除');
            $specialInfo->is_del = 1;
            $liveInfo->is_del = 1;
            if ($specialInfo->save() && $liveInfo->save()) {
                LiveStudio::where(['special_id' => $id_item])->update(['is_del' => 1]);
            }else{
    ...
    public function recommend($id = 0){
        if (!$id) return Json::fail('缺少参数');
        $specialInfo = SpecialModel::get($id);
        if (!$specialInfo) return Json::fail('课程不存在');
    ...
    public function download_list($special_id = 0)
    {
        ...
        if (!$specialInfo) return $this->failed('查找的课程不存在');
        if ($specialInfo->is_del) return $this->failed('课程已经删除');
    ...
    {
        $specialInfo = SpecialModel::get($special_id);
        if (!$specialInfo) return $this->failed('课程不存在');
        $liveInfo = LiveStudio::where('special_id', $special_id)->find();
    ...
    public function save_task($live_id = 0, $special_id = 0)
    {
        ...
        if ($special_id) {
            ...
            $f[] = Form::input('special_id', '课程id', $special_id)->disabled(true);
    ...
    public function set_task($live_id = 0)
    {
        ...
        if (!$data['special_id']) return Json::fail('关联课程Id缺失');
    ...
    public function create($id=0)
    {
        ...
        $form = Form::create(Url::build('save',['id'=>$id]), [
            ...
            Form::number('sort', '排序', isset($gift) ? $gift->sort : 0)->max(99999),
