...
class SpecialTaskCategory extends AuthController
{
    ...
    public function save($id = 0)
    {
        ...
        if ($id) {
            ...
            if(!$cate['pid'] && $post['pid'] && SpecialTaskCategoryModel::be(['pid'=>$id,'is_del'=>0])) return Json::fail('无法移动有下级的分类');
            if(SpecialTaskCategoryModel::where(['title'=>$post['title'],'is_del'=>0])->where('id','<>',$id)->count()>=1) return Json::fail('分类名称已存在');
