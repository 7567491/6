    ...
    public function updateRefundY(Request $request, $id)
    {
        ...
        if ($product['pay_type'] == 'weixin') {
            ...
            if(!empty($res) && $res == 10000){
                EventSignUpModel::checkTrans(true);
            }else{
                EventSignUpModel::checkTrans(false);
        ...
        if ($resEdit) {
            ...
            UserBill::income('活动订单退款', $product['uid'], $pay_type, 'pay_sign_up_refund', $refund_price, $product['id'], 0, '活动订单退款' . floatval($refund_price) . '元');
