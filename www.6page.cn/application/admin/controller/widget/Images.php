            ...
            try {
                if($file){
                    ...
                    if($info){
                        ...
                        // 成功上传后 获取上传信息
                        SystemAttachmentModel::attachmentAdd($info->getFilename(), 0, 'image/'.$info->getExtension(), $file_url, $file_url, $pid, 1, time());
                        return Json::successful(['url' => $info->getSaveName()]);
                    }else{
                        // 上传失败获取错误信息
                        echo $file->getError();
