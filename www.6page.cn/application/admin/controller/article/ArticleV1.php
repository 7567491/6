...
class ArticleV1 extends AuthController
{
    ...
    * */
    ...
    public function mp_recommend($article_id = 0)
    {
        if (!$article_id) $this->failed('缺少参数');
        $article = Article::get($article_id);
        if (!$article) $this->failed('没有查到此新闻');
        $form = Form::create(Url::build('save_mp_recommend', ['article_id' => $article_id]), [
            Form::select('recommend_id', '推荐')->setOptions(function () {
                $list = MpRecommend::where(['is_show' => 1, 'type' => 1])->field('title,id')->order('sort desc,add_time desc')->select();
                $menus = [];
                foreach ($list as $menu) {
                    $menus[] = ['value' => $menu['id'], 'label' => $menu['title']];
                }
     ...
     * 保存推荐
     * */
    public function save_mp_recommend($article_id = 0)
    {
        if (!$article_id) $this->failed('缺少参数');
        $data = parent::postMore([
            ['recommend_id', 0],
        ...
        $data['type'] = 1;
        $data['link_id'] = $article_id;
        if (MpRecommendRelation::be(['type' => 1, 'link_id' => $article_id, 'recommend_id' => $data['recommend_id']])) return Json::fail('已推荐,请勿重复推荐');
     ...
     * @param int $id
     */
    public function cancel_mp_recommendation($id=0,$article_id=0)
    {
        if (!$id || !$article_id) $this->failed('缺少参数');
        if (MpRecommendRelation::be(['id' => $id, 'link_id' => $article_id])){
            $res=MpRecommendRelation::where(['id'=>$id,'link_id'=>$article_id])->delete();
            if ($res)
                return Json::successful('取消推荐成功');
            else
                return Json::fail('取消推荐失败');
