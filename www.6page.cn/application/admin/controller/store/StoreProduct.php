...
class StoreProduct extends AuthController
{
     ...
     * @throws \think\exception\DbException
     */
    public function mp_recommend($product_id = 0)
    {
        if (!$product_id) $this->failed('缺少参数');
        $special = ProductModel::get($product_id);
        if (!$special) $this->failed('没有查到此课程');
        if ($special->is_del) $this->failed('此课程已删除');
        $form = Form::create(Url::build('save_mp_recommend', ['product_id' => $product_id]), [
            Form::select('recommend_id', '推荐')->setOptions(function () use ($product_id) {
                $list = MpRecommend::where(['is_show' => 1,'type'=>4])->where('is_fixed', 0)->field('title,id')->order('sort desc,add_time desc')->select();
                $menus = [];
                foreach ($list as $menu) {
                    $menus[] = ['value' => $menu['id'], 'label' => $menu['title']];
                }
     ...
     * @throws \think\exception\DbException
     */
    public function save_mp_recommend($product_id = 0)
    {
        if (!$product_id) $this->failed('缺少参数');
        $data = parent::postMore([
            ['recommend_id', 0],
        ...
        $data['type'] = $recommend->type;
        $data['link_id'] = $product_id;
        if (MpRecommendRelation::be(['type' => $recommend->type, 'link_id' => $product_id, 'recommend_id' => $data['recommend_id']])) return Json::fail('已推荐,请勿重复推荐');
     ...
     * @param int $id
     */
    public function cancel_mp_recommendation($id=0,$product_id=0)
    {
        if (!$id || !$product_id) $this->failed('缺少参数');
        if (MpRecommendRelation::be(['id' => $id, 'link_id' => $product_id])){
            $res=MpRecommendRelation::where(['id'=>$id,'link_id'=>$product_id])->delete();
            if ($res)
                return Json::successful('取消推荐成功');
            else
                return Json::fail('取消推荐失败');
