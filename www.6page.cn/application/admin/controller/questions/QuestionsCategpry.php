...
class QuestionsCategpry extends AuthController
{
    ...
    {
        $category=QuestionsCategpryModel::where(['pid'=>0,'is_del'=>0])->select();
    ...
    public function save($id = 0)
    {
        ...
        if ($id) {
            ...
            if(!$cate['pid'] && $post['pid'] && QuestionsCategpryModel::be(['pid'=>$id,'is_del'=>0])) return Json::fail('无法移动有下级的分类');
            if(QuestionsCategpryModel::where(['title'=>$post['title'],'is_del'=>0])->where('id','<>',$id)->count()>=1) return Json::fail('分类名称已存在');
            ...
            $post['add_time'] = time();
            if(QuestionsCategpryModel::be(['title'=>$post['title'],'is_del'=>0])){
    ...
    public function delete($id = 0)
    {
        ...
        $cate=QuestionsCategpryModel::get($id);
        if(!$cate['pid']){
            $count=QuestionsCategpryModel::where('pid',$id)->where('is_del',0)->count();
            if ($count) return Json::fail('暂无法删除,请删除下级分类');
        }
        if(QuestionsModel::where('pid',$id)->where('is_del',0)->count()) return Json::fail('暂无法删除,请先删除题目');
