...
class TestPaper extends AuthController
{
     ...
     * 获取试卷列表
     */
    public function getExaminationRecords($type=1,$test_id=0,$uid=0)
    {
        $where = parent::getMore([
            ['page', 1],
            ['limit', 20],
     ...
     * 用户答题
     */
    public function answers($record_id=0,$test_id=0,$type=1)
    {
    ...
    public function getTestPaperAnswers($test_id,$record_id)
    {
        ...
        return Json::successlayui(TestPaperQuestions::getExaminationRecordAnswers($where,$test_id,$record_id));
    }
    /**
     * 快速编辑
    ...
    public function save_add($id=0,$type=1){
        ...
        $data['item_number']=bcadd($data['single_number'],bcadd($data['many_number'],$data['judge_number'],1),1);
        $total_single_score=bcmul($data['single_number'],$data['single_score'],1);
        $total_many_score=bcmul($data['many_number'],$data['many_score'],1);
        $total_judge_score=bcmul($data['judge_number'],$data['judge_score'],1);
        $data['total_score']=bcadd($total_single_score,bcadd($total_many_score,$total_judge_score,1),1);
...
//                $data['member_money']
    ...
    public function delete($id=0)
    {
        ...
        foreach ($id_arr as $k => $id_item) {
            $test=TestPaperModel::get($id_item);
            if(!$test) return Json::fail('要删除的试卷不存在');
            $res =parent::getDataModification('test',$id_item,'is_del',1);
            if($res){
                TestPaperQuestions::where('test_id',$id_item)->delete();
            }else{
     ...
     * @throws \think\exception\DbException
     */
    public function mp_recommend($test_id = 0)
    {
        if (!$test_id) $this->failed('缺少参数');
        $testPaper= TestPaperModel::get($test_id);
        if (!$testPaper) $this->failed('没有查到此试卷');
        if ($testPaper->is_del) $this->failed('此试卷已删除');
        $type=$testPaper->type;
        $form = Form::create(Url::build('save_mp_recommend', ['test_id' => $test_id]), [
            Form::select('recommend_id', '推荐')->setOptions(function () use ($type) {
                $types=$type==1 ? 11 : 12;
                $list = MpRecommend::where(['is_show' => 1,'type'=>$types])->where('is_fixed', 0)->field('title,id')->order('sort desc,add_time desc')->select();
                $menus = [];
                foreach ($list as $menu) {
                    $menus[] = ['value' => $menu['id'], 'label' => $menu['title']];
                }
     ...
     * @throws \think\exception\DbException
     */
    public function save_mp_recommend($test_id = 0)
    {
        if (!$test_id) $this->failed('缺少参数');
        $data = parent::postMore([
            ['recommend_id', 0],
        ...
        $data['type'] = $recommend->type;
        $data['link_id'] = $test_id;
        if (MpRecommendRelation::be(['type' => $recommend->type, 'link_id' => $test_id, 'recommend_id' => $data['recommend_id']])) return Json::fail('已推荐,请勿重复推荐');
     ...
     * @param int $id
     */
    public function cancel_mp_recommendation($id=0,$test_id=0)
    {
        if (!$id || !$test_id) $this->failed('缺少参数');
        if (MpRecommendRelation::be(['id' => $id, 'link_id' => $test_id])){
            $res=MpRecommendRelation::where(['id'=>$id,'link_id'=>$test_id])->delete();
            if ($res)
                return Json::successful('取消推荐成功');
            else
                return Json::fail('取消推荐失败');
