...
class Questions extends AuthController
{
    ...
    public function save_add($id=0){
        ...
        if($id){
            ...
            $data['add_time']=time();
            if(QuestionsModel::be(['stem'=>$data['stem'],'is_del'=>0])){
    ...
    public function delete($id=0)
    {
        ...
        foreach ($id_arr as $k => $id_item) {
            $questions=QuestionsModel::get($id_item);
            if(!$questions) return Json::fail('要删除的试题不存在');
            $testPaperQuestions = TestPaperQuestions::where('questions_id',$id_item)->find();
            if ($testPaperQuestions) {
                $type = $testPaperQuestions['type'] == 1 ? '练习' : '考试';
                $test_id = $testPaperQuestions['test_id'];
                return Json::fail('该题目已在编号为'.$test_id.'的'.$type.'中使用，不能删除，请先将题目从'.$type.'中移除再操作');
            }
            $res =parent::getDataModification('questions',$id_item,'is_del',1);
            if(!$res){
                return Json::fail('删除失败');
            }
        }
