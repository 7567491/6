...
class SystemGroupData extends AuthController
{
    ...
    public function set_show($is_show = '', $id = '',$recommend = '')
    {
        ...
        } else if ($recommend=='mp'){
            $res = MpRecommend::where(['id' => $id])->update(['is_show' => (int)$is_show]);
        }else{
    ...
    public function set_value($field = '', $id = '', $value = '',$recommend = '')
    {
        ...
        if($recommend=='web'){
            ...
            $res = MpRecommend::where(['id' => $id])->update([$field => $value]);
        }else{
     ...
     * 个人中心菜单
     */
    public function center_menu($gid=0)
    {
        $this->assign('gid',$gid);
        return $this->fetch();
    }
     ...
     * @return \think\Response
     */
    public function index_mp($gid)
    {
        $mp_is_show_or_hide=SystemConfigService::get('mp_is_show_or_hide');
        $this->assign(compact("gid",'mp_is_show_or_hide'));
        return $this->fetch();
    }
    ...
    public function save_group_data($name = '')
    {
        ...
        if (!$data['select_id']) {
            return Json::fail('请选择' . ($data['type'] ? '分类' : "课程"));
        }
        ...
        if ($data['type']) {
            $info['wap_link']['value'] = '/m/course?cate_id=' . $data['select_id'];
        } else {
            $is_light=Special::where('id',$data['select_id'])->value('is_light');
            if($is_light){
                $info['wap_link']['value'] = '/m/single-course?id=' . $data['select_id'];
            }else{
                $info['wap_link']['value'] = '/m/view-course?id=' . $data['select_id'];
            }
        }
        if (isset($data['id']) && $data['id']) {
    ...
    public function mp_is_show_or_hide($value)
    {
        $res=SystemConfig::edit(['value' => json_encode($value)], 'mp_is_show_or_hide', 'menu_name');
        if ($res) {
            return Json::successful('编辑成功');
        } else {
            return Json::fail('编辑失败');
    ...
    // 小程序部分
    public function mp_recommend()
    {
        $this->assign('fixedList', MpRecommend::fixedList());
        return $this->fetch();
    }

    public function mp_recommend_list()
    {
        $where = parent::getMore([
            ['page', 1],
            ['limit', 20],
        ...
        return Json::successlayui(MpRecommend::getRecommendList($where));
    }


    public function create_mp_recommend($id = 0)
    {
        if ($id) $this->assign('recemmend_mp', MpRecommend::get($id));
        $this->assign('is_fixed', 1);
        $this->assign('grade_list', SpecialSubject::specialCategoryAll(1));
        $this->assign('id', $id);
        return $this->fetch();
    ...
    public function create_mp_recommend_v1($id = 0)
    {
        if ($id) $this->assign('recommend', MpRecommend::get($id));
        $this->assign('is_fixed', 0);
        $this->assign('grade_list', SpecialSubject::specialCategoryAll(1));
        $this->assign('is_fixed', 0);
        $this->assign('id', $id);
    ...
    public function save_mp_recommend($id = 0)
    {
        $post = parent::postMore([
            ['icon', ''],
            ['image', ''],
        ...
        if ($id) {
            $rescomm = MpRecommend::get($id);
            if (!$rescomm) return Json::fail('修改的信息不存在');
            MpRecommend::update($post, ['id' => $id]);
            return Json::successful('修改成功');
        } else {
            $post['add_time'] = time();
            if (MpRecommend::set($post))
                return Json::successful('保存成功');
            else
                return Json::fail('保存失败');
        }
    ...
    public function mp_recommend_content($id = 0)
    {
        if (!$id) return Json::fail('缺少参数');
        if ($this->request->isAjax()) {
            $where = parent::getMore([
            ...
            return Json::successlayui(MpRecommendRelation::getAll($where, $id));
        } else {
            $this->assign('id', $id);
            return $this->fetch();
        }
     ...
     * @return mixed
     */
    public function mp_recommend_article_content($id = 0)
    {
        $this->assign('id', $id);
        return $this->fetch();
    }
     ...
     * @return mixed
     */
    public function mp_recommend_test_content($id = 0)
    {
        $this->assign('id', $id);
        return $this->fetch();
    }
     ...
     * @return mixed
     */
    public function mp_recommend_store_content($id = 0)
    {
        $this->assign('id', $id);
        return $this->fetch();
    }

    public function mp_recommend_delete($id = 0)
    {
        if (!$id) return Json::fail('缺少参数');
        if (MpRecommendRelation::del($id))
            return Json::successful('删除成功');
        else
            return Json::fail('删除失败');
    }
     ...
     * @return json
     */
    public function set_mp_recommend_value($field = '', $id = '', $value = '')
    {
        $field == '' || $id == '' || $value == '' && Json::fail('缺少参数');
        if (MpRecommendRelation::where(['id' => $id])->update([$field => $value]))
            return Json::successful('保存成功');
        else
            return Json::fail('保存失败');
    }
     ...
     * @return \think\Response
     */
    public function delete_mp_recomm($id)
    {
        if (MpRecommendRelation::be(['recommend_id' => $id])) return Json::fail('删除失败，请先删除内容管理里面的列表');
        if (!MpRecommend::del($id))
            return Json::fail('删除失败');
        else
            return Json::successful('删除成功!');
    }
     ...
     * 个人中心菜单
     */
    public function mp_center_menu($gid=0)
    {
        $this->assign('gid',$gid);
        return $this->fetch();
    }
     ...
     * @throws \think\exception\DbException
     */
    public function create_mp_recommend_custom($id = 0,$is_fixed=1)
    {
        if ($id) {
            $mp_recommend = MpRecommend::get($id);
            if (!$mp_recommend) {
                return $this->failed('您修改的导航不存在');
            }
        }
        $f[] = Form::input('title', '名称', isset($mp_recommend) ? $mp_recommend->title : '');
        $f[] = Form::frameImageOne('icon', '图标(100*100px)', get_image_Url('icon'), isset($mp_recommend) ? $mp_recommend->icon : '')->icon('image')->width('100%')->height('500px');
        $f[] = Form::input('link', '跳转路径', isset($mp_recommend) ? $mp_recommend->link : '');
        $f[] = Form::input('sort', '排序', isset($mp_recommend) ? $mp_recommend->sort : 0);
        if($is_fixed==2){
            $f[] = Form::radio('is_promoter', '推广权限', isset($mp_recommend) ? $mp_recommend->is_promoter : 0)->options([['value' => 1, 'label' => '需要权限'], ['value' => 0, 'label' => '无需权限']]);
            $f[] = Form::radio('is_write_off', '核销权限', isset($mp_recommend) ? $mp_recommend->is_write_off : 0)->options([['value' => 1, 'label' => '需要权限'], ['value' => 0, 'label' => '无需权限']]);
        }
        $f[] = Form::radio('is_show', '状态', isset($mp_recommend) ? $mp_recommend->is_show : 0)->options([['value' => 1, 'label' => '显示'], ['value' => 0, 'label' => '隐藏']]);
        $form = Form::make_post_form('编辑', $f, Url::build('save_mp_recommend_custom', compact('id','is_fixed')), 2);
        $this->assign(compact('form'));
        return $this->fetch('public/form-builder');
    }
     ...
     * @param int $id
     */
    public function save_mp_recommend_custom($id = 0,$is_fixed=1)
    {
        $data = parent::postMore([
            ['title', ''],
            ['icon', ''],
        ...
        if ($id) {
            MpRecommend::edit($data,$id);
            return Json::successful('修改成功');
        } else {
            $data['add_time'] = time();
            if(MpRecommend::be(['title'=>$data['title'],'link'=>$data['link']]))return Json::fail('名称已存在');
            $res = MpRecommend::set($data);
            if ($res) {
                return Json::successful('添加成功');
            } else {
                return Json::fail('修改失败');
