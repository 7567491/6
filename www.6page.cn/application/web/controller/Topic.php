...
use app\web\model\topic\TestPaperQuestions;
...
use app\web\model\topic\CertificateRelated;
use app\web\model\topic\Relation;
...
class Topic extends AuthController
{
    ...
    {
        if (!$id) $this->failed('缺少参数,无法访问', Url::build('/index'));
        ...
        if (in_array($testPaper['money'], [0, 0.00]) || in_array($testPaper['pay_type'], [PAY_NO_MONEY])) {
            $res=true;
            if(!((!$this->uid || $this->uid == 0) ? false : TestPaperObtain::PayTestPaper($id,$this->uid, $testPaper['type']))){
                $res=TestPaperObtain::setUserTestPaper('',$id,$this->uid,$testPaper['type'],2);
            }
            if($res) $isPay = 1;
            else $isPay = 0;
        }else{
            $isPay = (!$this->uid || $this->uid == 0) ? false : TestPaperObtain::PayTestPaper($id,$this->uid,$testPaper['type']);
        }
        $testPaper['isPay']=$isPay;
        $user_level = !$this->uid ? 0 : $this->userInfo;
        $this->assign([
                'uid' => $this->uid,
                'titles' => $testPaper->title,
                'exam' => $testPaper,
    ...
    {
        if (!$test_id) $this->failed('缺少参数,无法访问', Url::build('/index'));
     ...
     * @return mixed
     */
    public function problem_sheet($test_id)
    {
        if (!$test_id) $this->failed('缺少参数,无法访问', Url::build('/index'));
        $title=TestPaper::PreExercisesWhere()->where('id',$test_id)->value('title');
        $this->assign(['uid'=>$this->uid,'title'=>$title]);
        return $this->fetch();
    }
     ...
     * @return mixed
     */
    public function question_sheet($test_id)
    {
        if (!$test_id) $this->failed('缺少参数,无法访问', Url::build('/index'));
        $title=TestPaper::PreExercisesWhere()->where('id',$test_id)->value('title');
        $this->assign(['uid'=>$this->uid,'title'=>$title]);
     ...
     * @param int $id
     */
    public function question_index($id=0)
    {
    ...
    public function userAnswer($test_id=0,$type=1)
    {
        ...
        $uid=$this->uid;
        $count=ExaminationRecord::where(['test_id'=>$testPaper['id'],'type'=>$testPaper['type'],'uid'=>$this->uid])->count();
        if($count>=$testPaper['frequency'] && $testPaper['frequency']>0 && $testPaper['type']==2) return JsonService::fail('您的考试次数已用完');
    ...
    public function testPaperQuestions($test_id,$type,$record_id=0)
    {
        ...
        $testPaper=TestPaper::PreExercisesWhere()->where('id',$test_id)->find();
        if(!$testPaper) return JsonService::fail('试卷不存在');
        $single=TestPaperQuestions::getQuestionslist($test_id,$type,1);
        $many=TestPaperQuestions::getQuestionslist($test_id,$type,2);
        $judge=TestPaperQuestions::getQuestionslist($test_id,$type,3);
        $question=array_merge($single,$many,$judge);
    ...
    public function answerSheet($test_id,$type,$record_id=0)
    {
        ...
        $many=TestPaperQuestions::getQuestionslist($test_id,$type,2);
        $judge=TestPaperQuestions::getQuestionslist($test_id,$type,3);
        $testPaperQuestion=array_merge($single,$many,$judge);
        $record=ExaminationRecord::where(['id'=>$record_id,'test_id'=>$test_id,'type'=>$type,'uid'=>$this->uid])->order('id desc')->find();
    ...
    public function specialTestPaper()
    {
        ...
        $test_ids=Relation::setWhere($relationship,$special_id)->order('sort desc,id desc')->column('relation_id');
        if(count($test_ids)){
            $data=TestPaper::PreExercisesWhere()->where('type',$type)->where('id','in',$test_ids)->orderRaw("FIELD(id, " . implode(',', $test_ids) . ")")->select();
