...
class DataDownloadOrder extends ModelBasic
{
    ...
    public static function createDataDownloadOrder($data,$uid,$payType,$total_num = 1)
    {
        ...
        $order=self::where(['uid'=>$uid,'data_id'=>$data->id,'paid'=>1,'is_del'=>0,'refund_status'=>0])->find();
        if($order) return self::setErrorInfo('您已购买资料，无需再次购买!');
        $orderInfo = [
            'uid' => $uid,
     ...
     * @throws \think\exception\DbException
     */
    public static function nativeDataDownloadPay($orderId,$field = 'order_id')
    {
        if(is_string($orderId))
        ...
        return WechatService::nativePay(null,$orderInfo['order_id'],$orderInfo['pay_price'],'datadownload',self::getSubstrUTf8($site_name.'-资料购买',30),'','NATIVE');
    }
    /**资料购买支付宝扫码支付
     * @param $orderId
     * @param string $field
     ...
     * @throws \think\exception\DbException
     */
    public static function alipayAativeDataDownloadPay($orderId,$field = 'order_id')
    {
        if(is_string($orderId))
            $orderInfo = self::where($field,$orderId)->find();
        else
        ...
        if($orderInfo['paid']) exception('支付已支付!');
        if($orderInfo['pay_price'] <= 0) exception('该支付无需支付!');
        return AlipayTradeWapService::init()->AliPayNative($orderInfo['order_id'], $orderInfo['pay_price'], '资料购买', 'datadownload');
    }
    /**资料余额支付
     * @param $order_id
     * @param $uid
    ...
    public static function yueDataDownloadPay($order_id,$uid)
    {
        ...
        $res2 = UserBill::expend('购买资料', $uid, 'now_money', 'pay_data_download', $orderInfo['pay_price'], $orderInfo['id'], $userInfo['now_money'], '余额支付' . floatval($orderInfo['pay_price']) . '元购买资料');
    ...
    public static function payDataDownloadSuccess($orderId)
    {
        ...
        try{
            WechatTemplateService::sendTemplate(WechatUser::where('uid', $order['uid'])->value('openid'), WechatTemplateService::ORDER_PAY_SUCCESS, [
                ...
                'remark' => '点击查看订单详情'
            ], $site_url . Url::build('/m/my-virtual'));
