...
class DataDownloadBuy extends ModelBasic
{
    ...
    {
        $add_time=time();
        if (self::be([ 'uid' => $uid, 'data_id' => $data_id])) return false;
    ...
    public static function getDataDownloadWhere($uid)
    {
        return self::alias('b')->join('DataDownload d','b.data_id=d.id')->where(['b.uid'=>$uid,'b.is_del'=>0,'d.is_del'=>0,'d.is_show'=>1]);
    }
    ...
    {
        $data=self::getDataDownloadWhere($uid)
            ->field('d.id,b.uid,b.data_id,b.type,d.member_money,b.add_time,d.title,d.image,d.money,d.pay_type,d.sales,d.ficti')
            ->page((int)$page,(int)$limit)->order('b.add_time DESC')->select();
        $data=count($data) > 0 ? $data->toArray() : [];
        $count=self::getDataDownloadWhere($uid)->count();
