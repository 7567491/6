...
class TestPaperOrder extends ModelBasic
{
    ...
    public static function createTestPaperOrder($testPaper,$uid,$payType,$total_num = 1)
    {
        ...
        $order=self::where(['uid'=>$uid,'test_id'=>$testPaper->id,'paid'=>1,'is_del'=>0,'refund_status'=>0])->find();
        if($order) return self::setErrorInfo('您已购买试卷，无需再次购买!');
        $orderInfo = [
            'uid' => $uid,
    ...
    public static function nativeTestPaperPay($orderId,$field = 'order_id')
    {
        if(is_string($orderId))
        ...
        return WechatService::nativePay(null,$orderInfo['order_id'],$orderInfo['pay_price'],'testpaper',self::getSubstrUTf8($site_name.'-考试购买',30),'','NATIVE');
    }
    ...
    public static function yueTestPaperPay($order_id,$uid)
    {
        ...
        $res2 = UserBill::expend('购买试卷', $uid, 'now_money', 'pay_test_paper', $orderInfo['pay_price'], $orderInfo['id'], $userInfo['now_money'], '余额支付' . floatval($orderInfo['pay_price']) . '元购买试卷');
     ...
     * @throws \think\exception\DbException
     */
    public static function alipayNativeTestPaperPay($orderId,$field = 'order_id')
    {
        if(is_string($orderId))
            $orderInfo = self::where($field,$orderId)->find();
        else
        ...
        if($orderInfo['paid']) exception('支付已支付!');
        if($orderInfo['pay_price'] <= 0) exception('该支付无需支付!');
        return AlipayTradeWapService::init()->AliPayNative($orderInfo['order_id'], $orderInfo['pay_price'], '考试购买', 'testpaper');
    }
    ...
    public static function payTestPaperSuccess($orderId)
    {
        ...
        // 查询试卷信息
        $testPaper = TestPaper::PreExercisesWhere()->find($order['test_id']);
        $type = $testPaper['type'];
        ...
        if($res1){
            try {
                ...
                TestPaperObtain::setUserTestPaper($orderId,$order['test_id'], $order['uid'], $type,1);
            } catch (\Throwable $e) {
            }
        }
        ...
        try{
            ...
            if($wechat_notification_message==1){
                WechatTemplateService::sendTemplate(WechatUser::where('uid', $order['uid'])->value('openid'), WechatTemplateService::ORDER_PAY_SUCCESS, [
                    ...
                    'remark' => '点击查看订单详情'
                ], $site_url . Url::build('/my') . '?page=exam');
                ...
                RoutineTemplate::sendOrderSuccess($data,$order['uid'],$site_url . Url::build('/my') . '?page=exam');
