...
class WechatService
{
    ...
    public static function options()
    {
        ...
        // 判断是否小程序
        $is_mp = isset($app_type) && $app_type == 'weapp';
        ...
        $mp = SystemConfigService::more(['mp_app_id','mp_app_secret']);

        $wechat_appid = isset($wechat['wechat_appid']) ? $wechat['wechat_appid']:'';
        $mp_appid = isset($mp['mp_app_id']) ? $mp['mp_app_id']:'';

        $config = [
            'app_id'=> $is_mp ? $mp_appid : $wechat_appid,
        ...
        $config['mini_program'] = [
            'app_id'=> $mp_appid,
            'secret'=>isset($mp['mp_app_secret']) ? $mp['mp_app_secret']:'',
        ];
        return $config;
    }
     ...
     * 回复文章消息
     * @param string|array $title 标题
     * @param string $thumb_media_id 图文消息的封面图片课节id（必须是永久 media_ID）
     ...
     * @param int $need_open_comment 是否打开评价，0不打开，1打开
     * @param int $only_fans_can_comment 是否粉丝才可评价，0所有人可评价，1粉丝才可评价
     * @return Article
     */
