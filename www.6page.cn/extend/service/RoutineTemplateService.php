...
class RoutineTemplateService extends AuthBasic{
    ...
    public static function sendTemplate($openId = '',$templateId = '',$link = '',$dataKey = array()){
        ...
        // 对$dataKey进行处理，thing.DATA不能超过20个字符
        foreach ($dataKey as $key => $item) {
            if (strpos($key, 'thing') !== false) {
                if (mb_strlen($dataKey[$key]['value'], 'utf-8') > 20) {
                    $dataKey[$key]['value'] = mb_substr($dataKey[$key]['value'], 0, 20, 'utf-8');
                }
            }
        }
        $data['data'] =  $dataKey;  //模板内容，不填则下发空模板
        if(self::TEMPLATE_CHECK!=1) return false;
        $res = parent::curlPost($url, json_encode($data));
