...
//        WechatUser::saveUser($message->FromUserName);
    ...
    public static function wechatEventScanSubscribe($message, $eventKey = '')
    {
        ...
        // 处理eventkey
        // 移除无用前缀
        $eventKey_handled = str_replace('qrscene_', '', $message->EventKey);
        if ($eventKey_handled && strpos($eventKey_handled, 'wechat_login__') === 0) {
            return HookService::resultListen('wechat_qrcode_login_subscribe', $message, $eventKey_handled, true, QrcodeEventBehavior::class);
        }
    ...
    public static function wechatEventScan($message)
    {
        ...
        $eventKey_handled = str_replace('qrscene_', '', $message->EventKey);
        if ($eventKey_handled && strpos($eventKey_handled, 'wechat_login__') === 0) {
            return HookService::resultListen('wechat_qrcode_login_scan', $message, $eventKey_handled, true, QrcodeEventBehavior::class);
        }
        ...
        if($message->EventKey){
            WechatUser::where('openid',$message->FromUserName)->update(['scan_key'=>$message->EventKey]);
        }
        return WechatReply::reply('subscribe');
    }
