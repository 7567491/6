...
class AliyunLive extends AliyunSdk
{
    ...
    public function foundLiveStudio($streamName, $expire = 1800)
    {
        ...
        //flv 播放地址
        $flvUrl = $this->protocol . $this->playLike . $LiveName . '.flv?auth_key=' . md5($LiveName . '.flv-' . $time . '-0-0-' . $this->payKey);
        //m3u8 播放地址
        $m3u8Url = $this->protocol . $this->playLike . $LiveName . '.m3u8?auth_key=' . md5($LiveName . '.m3u8-' . $time . '-0-0-' . $this->payKey);
