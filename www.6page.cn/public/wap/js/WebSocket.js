(function (global) {
    ...
    var socket = {
        ...
        connect: function () {
            ...
            that.ws = new WebSocket(protocol + document.domain + "/wyws" +'?uid='+window.uid+'&room='+window.room);//这里如果使用127.0.0.1或者localhost会出现连接失败。当时为了方便以后的维护，这里在php的全局文件里定义了一个常量来定义ip，后来本地开发完提交到linux服务器环境之后发现链接失败！按照此行代码会有效连接~
