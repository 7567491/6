(function(global,factory){
    ...
    var $h = $h || {};
    $h._errorMsgOptions = {offset: '30%',anim: 2,time:1200,shadeClose:true,shade:'0.1'};
    ...
    $h.getMsgHthml=function (title,icon) {
        ...
        if(icon) html+='<img class="msg_image" src="/mp/images/msg_image/'+icon+'.png">';
    ...
    $h.U = function(opt){
        var m = opt.m || 'mp',c = opt.c || 'public_api', a = opt.a || 'index',q = opt.q || '',p = opt.p || {},_params = '',gets='',ext=opt.ext || '.html';
        var is_short = opt.s || false
        ...
        var path = "/"+m+"/"+c+"/"+a+(_params == '' ? '' : '/'+_params)+ext+(gets == '' ? q : '?'+gets);
        var path_map = [

            ['mp','mp/index/index'],
            ['mp/search','mp/index/search'],
            ['mp/course','mp/special/special_cate'],
            ['mp/view-course','mp/special/details'],
            ['mp/single-course','mp/special/single_details'],
            ['mp/single-course-txt','mp/special/single_text_detail'],
            ['mp/shop','mp/store/index'],
            ['mp/view-virtual','mp/special/data_details'],
            ['mp/view-product','mp/store/detail'],
            ['mp/my','mp/my/index'],
            ['mp/teachers','mp/special/teacher_list'],
            ['mp/view-teacher','mp/special/teacher_detail'],
            ['mp/articles','mp/article/news_list'],
            ['mp/view-article','mp/special/teacher_list'],
            ['mp/lesson','mp/special/task_info'],
            ['mp/lesson-txt','mp/special/task_text_info'],
            ['mp/live-room','mp/live/index'],
            ['mp/my-courses','mp/special/grade_special'],
            ['mp/my-record','mp/special/record'],
            ['mp/my-order','mp/special/order_store_list'],
            ['mp/my-wrong','mp/topic/question_wrong'],
            ['mp/my-topic','mp/topic/question_user'],
            ['mp/my-sign','mp/my/sign_in'],
            ['mp/my-vip','mp/special/member_recharge'],
            ['mp/my-spread','mp/spread/spread'],
            ['mp/pink-order','mp/my/order_list'],
            ['mp/my-activity','mp/my/sign_list'],
            ['mp/my-gift','mp/my/my_gift'],
            ['mp/my-bill','mp/my/bill_detail'],
            ['mp/recharge','mp/special/recharge_index'],
            ['mp/my-certificate','mp/topic/certificate_list'],
            ['mp/view-certificate','mp/topic/certificate_detail'],
            ['mp/my-fav','mp/special/grade_list'],
            ['mp/my-address','mp/my/address'],
            ['mp/edit-address','mp/my/edit_address'],
            ['mp/my-virtual','mp/material/my_material'],
            ['mp/about','mp/my/about_us'],
            ['mp/my-profile','mp/my/user_info'],
            ['mp/test','mp/topic/problem_index'],
            ['mp/view-test','mp/topic/problem_detail'],
            ['mp/test-sheet','mp/topic/problem_sheet'],
            ['mp/test-result','mp/topic/problem_result'],
            ['mp/all-test','mp/topic/test_category'],
            ['mp/exam','mp/special/question_index'],
            ['mp/view-exam','mp/topic/question_detail'],
            ['mp/exam-sheet','mp/topic/question_sheet'],
            ['mp/exam-result','mp/topic/question_result'],
            ['mp/all-exam','mp/topic/question_category'],
            ['mp/view-wrong','mp/topic/question_detail_wrong'],
            ['mp/view-certificate','mp/topic/certificate_detail'],
            ['mp/view-activity','mp/my/sign_my_order'],
            ['mp/nice','mp/index/unified_list'],
            ['mp/pink','mp/special/group_list'],
            ['mp/login','mp/login/index'],
            ['mp/service-list','mp/service/service_list']
        ];
        // 短链接替换
        if (is_short) {
            for (var path_index = 0; path_index < path_map.length; path_index++) {
                if (path.indexOf(path_map[path_index][1]) !== -1) {
                    return path.replace(path_map[path_index][1], path_map[path_index][0])
                }
            }
        }
        return path;
    ...
    $h.isLogin = function(){
        return !!parseInt($h.getCookie('is_login'));
    };
